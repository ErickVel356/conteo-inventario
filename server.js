...
══════════════════════════════════════════════════════════════ -->
<!-- ══════════════════════════════════════════════════════════════
     SELECCIÓN DE GRUPO — HPC / Cemaco (agregado ago-2026)
     Esta es la PRIMERA pantalla que ve cualquiera al entrar, antes
     del gate de clave y del login por nombre. HPC revela el flujo
     que ya existía (gate -> login -> menú de 3 módulos), sin ningún
     cambio. Cemaco no tiene módulos aún, así que no pasa por gate
     ni login: solo muestra un mensaje.
══════════════════════════════════════════════════════════════ -->
<div id="pg-grupo" style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:#f0f0ee;padding:1rem">
  <div style="background:#fff;border:1px solid #e0e0e0;border-radius:16px;padding:2rem;width:100%;max-width:420px;box-shadow:0 4px 24px rgba(0,0,0,.08)">
    <div style="text-align:center;margin-bottom:1.5rem">
      <div style="font-size:2.5rem;margin-bottom:8px">📦</div>
      <h2 style="font-size:1.2rem;font-weight:700;margin-bottom:4px">Conteo Físico</h2>
      <p style="font-size:.85rem;color:#888">¿Con qué grupo vas a trabajar?</p>
    </div>
    <div class="menu-grid" style="margin-top:0">
      <div class="menu-card" id="mc-grupo-hpc">
        <div class="mc-icon">🏭</div>
        <div class="mc-title">HPC</div>
        <div class="mc-sub">Ingresar</div>
      </div>
      <div class="menu-card" id="mc-grupo-cemaco">
        <div class="mc-icon">🏬</div>
        <div class="mc-title">Cemaco</div>
        <div class="mc-sub">Ingresar</div>
      </div>
    </div>
    <div style="text-align:center;margin-top:1.25rem;font-size:.7rem;color:#999;font-family:monospace">Versión: v5.2.32 · 13-jun</div>
  </div>
</div>

<!-- PÁGINA CEMACO — vacía por ahora, sin gate ni login -->
<div id="pg-cemaco" style="min-height:100vh;display:none;align-items:center;justify-content:center;background:#f0f0ee;padding:1rem">
  <div style="background:#fff;border:1px solid #e0e0e0;border-radius:16px;padding:2.5rem 2rem;width:100%;max-width:380px;box-shadow:0 4px 24px rgba(0,0,0,.08);text-align:center">
    <div style="font-size:2.6rem;margin-bottom:1rem">🏬</div>
    <p style="font-size:1.05rem;font-weight:700;color:#333;margin-bottom:.4rem">Cemaco</p>
    <p style="font-size:.85rem;color:#888">Todavía no hay módulos disponibles para este grupo.</p>
    <button id="btn-cemaco-volver" style="margin-top:1.5rem;padding:10px 20px;font-size:.85rem;background:#f0f0f0;border:1px solid #ddd;border-radius:10px;cursor:pointer">‹ Volver</button>
  </div>
</div>
...
<!-- ══════════════════════════════════════════════════════════════
     ...
     FIX (ago-2026): el gate ahora arranca OCULTO — solo se muestra cuando
     el usuario elige el grupo "HPC" en #pg-grupo. Antes era la primera
     pantalla visible de toda la app.
══════════════════════════════════════════════════════════════ -->
<div id="pg-gate" style="min-height:100vh;display:none;align-items:center;justify-content:center;background:#f0f0ee;padding:1rem">
  <div style="background:#fff;border:1px solid #e0e0e0;border-radius:16px;padding:2rem;width:100%;max-width:360px;box-shadow:0 4px 24px rgba(0,0,0,.08)">
    ...
    <div style="text-align:center;margin-top:1rem;font-size:.7rem;color:#999;font-family:monospace">Versión: v5.2.32 · 13-jun</div>
  </div>
</div>

<!-- ══════════════════════════════════════════════════════════════
     GATE CEMACO — clave de acceso propia del grupo Cemaco (ago-2026)
     - Mismo mecanismo que el gate de HPC: clave vive en Supabase
       (key='app_config', campo "cemacoKey"), default 'cemaco2026' si
       el campo aún no existe. Se cambia desde Supabase igual que las
       otras claves (accessKey, bodegaKey, rgisKey).
     - Se recuerda por el día en sessionStorage (mismo criterio que HPC):
       recargar el MISMO día no vuelve a pedir la clave de Cemaco.
     - Al pasar, lleva directo a #pg-cemaco (que hoy está vacío).
══════════════════════════════════════════════════════════════ -->
<div id="pg-gate-cemaco" style="min-height:100vh;display:none;align-items:center;justify-content:center;background:#f0f0ee;padding:1rem">
  <div style="background:#fff;border:1px solid #e0e0e0;border-radius:16px;padding:2rem;width:100%;max-width:360px;box-shadow:0 4px 24px rgba(0,0,0,.08)">
    <div style="text-align:center;margin-bottom:1.5rem">
      <div style="font-size:2.5rem;margin-bottom:8px">🔒</div>
      <h2 style="font-size:1.2rem;font-weight:700;margin-bottom:4px">Acceso restringido</h2>
      <p style="font-size:.85rem;color:#888">Ingresa la clave de acceso de Cemaco</p>
    </div>
    <div style="margin-bottom:1rem">
      <input id="gate-cemaco-pass" type="password" autocomplete="off" placeholder="Clave de acceso"
        style="display:block;width:100%;padding:12px 14px;font-size:1rem;border:1px solid #ccc;border-radius:10px;background:#fff;min-height:48px;font-family:inherit;box-sizing:border-box">
    </div>
    <div id="gate-cemaco-err" style="display:none;color:#c0392b;font-size:.85rem;margin-bottom:.75rem;padding:8px 12px;background:#fdecea;border-radius:8px"></div>
    <button id="btn-gate-cemaco-enter" style="display:block;width:100%;padding:13px;font-size:1rem;font-weight:700;background:#1a1a1a;color:#fff;border:none;border-radius:10px;cursor:pointer;min-height:48px;font-family:inherit">Continuar</button>
    <div id="gate-cemaco-loading" style="display:none;text-align:center;margin-top:1rem;font-size:.8rem;color:#888">Verificando…</div>
    <button id="btn-gate-cemaco-volver" style="display:block;width:100%;margin-top:.75rem;padding:10px;font-size:.82rem;background:none;border:none;color:#888;cursor:pointer">‹ Volver</button>
    <div style="text-align:center;margin-top:1rem;font-size:.7rem;color:#999;font-family:monospace">Versión: v5.2.32 · 13-jun</div>
  </div>
</div>
...
══════════════════════════════════════════════════════════════ -->
    <!-- RGIS: Pantalla nombre real obligatorio -->
    <div id="pg-rgis-nombre" style="min-height:100vh;display:none;align-items:center;justify-content:center;background:#f0f0ee;padding:1rem">
      <div style="background:#fff;border:1px solid #e0e0e0;border-radius:16px;padding:2rem;width:100%;max-width:380px;box-shadow:0 4px 24px rgba(0,0,0,.08)">
        <div style="text-align:center;margin-bottom:1.2rem">
          <div style="font-size:2rem;margin-bottom:6px">&#128100;</div>
          <h2 style="font-size:1.1rem;font-weight:700;margin-bottom:2px">Identificaci&oacute;n RGIS</h2>
          <p style="font-size:.83rem;color:#888">Ingresaste como <strong id="rgis-op-label">Operador ?</strong></p>
        </div>
        <label style="font-size:.85rem;font-weight:600;display:block;margin-bottom:.4rem">Tu nombre completo <span style="color:#c0392b">*</span></label>
        <input type="text" id="rgis-nombre-real" placeholder="Ej: Carlos L&oacute;pez"
          style="width:100%;padding:.6rem .8rem;font-size:.95rem;border:1.5px solid #ddd;border-radius:8px;box-sizing:border-box;margin-bottom:.4rem"
          autocomplete="off" autocorrect="off" autocapitalize="words">
        <div id="rgis-nombre-err" style="color:#c0392b;font-size:.8rem;margin-bottom:.5rem;display:none">El nombre es obligatorio.</div>
        <button id="rgis-nombre-btn" style="width:100%;padding:.7rem;font-size:.95rem;font-weight:700;background:#111;color:#fff;border:none;border-radius:8px;cursor:pointer;margin-top:.3rem">Continuar &rarr;</button>
        <button id="rgis-nombre-volver" style="width:100%;padding:.5rem;font-size:.83rem;background:none;border:none;color:#888;cursor:pointer;margin-top:.4rem">&larr; Volver a selecci&oacute;n</button>
      </div>
    </div>
...
<div id="main-wrap" style="display:none">
  ...
  <div class="topbar" id="global-topbar">
    ...
    <span id="app-version-badge" style="font-size:.68rem;background:#e8f4ff;padding:3px 8px;border-radius:14px;color:#0066cc;white-space:nowrap;font-weight:600;font-family:monospace" title="Versión del cliente. Si pide ayuda, mostrá esta versión.">Versión: v5.2.32 · 13-jun</span>
  ...
  <div class="wrap">
    ...
    <!-- ── MÓDULO BODEGA CDG v3 ─────────────────────────────────────────────
         Reconstrucción desde cero jun-2026. Fuente: Bloqque_Bodega_CDG.docx
         No usa /api/state. Tablas: bod_sesiones, bod_lineas, bod_teorico_952,
         bod_manifiestos_control, bod_papel_trabajo, bod_licencia_hija, sku_catalog
    ─────────────────────────────────────────────────────────────────────── -->

    <!-- MENÚ BODEGA CDG -->
    <div id="pg-bod-menu" class="page">
      <p style="font-size:.9rem;color:#666;margin-bottom:.75rem">Módulo Bodega CDG</p>
      <div class="submenu-list">
        <div class="submenu-item" id="sm-bod-captura">
          <div class="si-icon">📦</div>
          <div class="si-body"><div class="si-title">Captura de Tarima</div><div class="si-sub">Registrar tarimas y SKUs por operador</div></div>
        </div>
        <div class="submenu-item" id="sm-bod-auditoria" style="display:none">
          <div class="si-icon">🔍</div>
          <div class="si-body"><div class="si-title">Auditar Sesión</div><div class="si-sub">Tarimas · Detalle · Teórico · Manifiestos · Papel de Trabajo</div></div>
        </div>
        <div class="submenu-item" id="sm-bod-barra-sku" style="display:none">
          <div class="si-icon">📋</div>
          <div class="si-body"><div class="si-title">Catálogo Barra-SKU</div><div class="si-sub">Base de datos de códigos y SKUs</div></div>
        </div>
      </div>
    </div>
    ...
    <div id="pg-bod-barra-sku" class="page">
      <div id="bod-barra-contenido"></div>
    </div>

    <!-- CAPTURA DE TARIMA -->
    <div id="pg-bod-captura" class="page">
      <div id="bod-captura-contenido"></div>
    </div>
    ...
    <div id="pg-hamilton" class="page">
      <div class="submenu-list">
        ...
        <div class="submenu-item" id="sm-boomroom">
          <div class="si-icon">🏗</div>
          <div class="si-text">
            <div class="si-title">BoomRooms</div>
            <div class="si-sub">Ubicaciones de almacenamiento Hamilton</div>
          </div>
          <span class="submenu-badge badge" style="background:#f0e6ff;color:#6a0dad" id="cnt-boomroom">0</span>
        </div>
      </div>
    ...
    <div id="pg-cdg-v2-licencia" class="page">
      ...
      <div class="card" id="cdgv2-lic-header" style="margin-bottom:.75rem;padding:.85rem 1rem">
        <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:8px;flex-wrap:wrap">
          ...
          <div style="display:flex;gap:6px;flex-wrap:wrap;align-items:center">
            ...
            <button id="btn-cdgv2-editar-lic" class="btn btn-sm"
              style="display:none;background:#e8eaf6;color:#3949ab;border:1px solid #9fa8da;font-size:.78rem">
              ✏️ Editar
            </button>
      ...
      <!-- Formulario editar licencia (colapsable) -->
      <div id="cdgv2-editar-form" style="display:none;margin-bottom:.75rem">
        <div class="card" style="padding:.85rem 1rem;border:2px solid #9fa8da">
          <div style="font-size:.78rem;font-weight:600;color:#3949ab;margin-bottom:.6rem;text-transform:uppercase;letter-spacing:.3px">✏️ Editar licencia</div>
          <div class="cdg-row" style="flex-wrap:wrap">
            <div class="cdg-field" style="flex:2;min-width:140px">
              <label>Correlativo / Nombre</label>
              <input type="text" class="field-inp" id="cdgv2-edit-nombre" placeholder="Ej: U25-654321">
            </div>
            <div class="cdg-field" style="flex:1;min-width:100px">
              <label>Tipo de bodega</label>
              <select id="cdgv2-edit-tipo" class="field-inp">
                <option value="CDG">CDG</option>
                <option value="KTM">KTM</option>
                <option value="otro">Otro</option>
              </select>
            </div>
            <div class="cdg-field" style="justify-content:flex-end;gap:6px">
              <label>&nbsp;</label>
              <div style="display:flex;gap:6px">
                <button class="btn btn-sm" id="btn-cdgv2-editar-cancel" style="background:#f5f5f3;color:#555">Cancelar</button>
                <button class="btn btn-sm btn-dark" id="btn-cdgv2-editar-ok" style="background:#3949ab;min-width:80px">Guardar</button>
              </div>
            </div>
          </div>
          <div id="cdgv2-editar-msg" style="font-size:.82rem;margin-top:.4rem"></div>
        </div>
      </div>
      ...
      <div id="cdgv2-add-form" style="background:#fafaf8;border:1px solid #e0e0e0;border-radius:10px;padding:.75rem .9rem;margin-bottom:.75rem">
        ...
        <div class="cdg-row" style="margin-bottom:0;flex-wrap:wrap">
          <div class="cdg-field" style="flex:1;min-width:90px;position:relative">
            <label>SKU</label>
            <input type="text" class="field-inp" id="cdgv2-add-sku" placeholder="SKU o C999.###.##" inputmode="text" autocomplete="off" autocorrect="off" autocapitalize="characters">
          ...
          <div class="cdg-field" style="min-width:80px">
            <label>Tarima</label>
            <input type="number" class="field-inp" id="cdgv2-add-tarima" min="1" max="100" placeholder="1-100" inputmode="numeric">
          </div>
          <div class="cdg-field" style="min-width:90px">
            <label>Costo unit.</label>
    ...
    <!-- ── PÁGINA: BOOMROOMS ─────────────────────────────────── -->
    <div id="pg-boomroom" class="page">

      <!-- 4 pestañas igual que Embarques -->
      <div class="tab-pills" id="br-tab-pills">
        <button class="tab-pill on" id="br-tab-btn-conteo">📋 Conteo</button>
        <button class="tab-pill" id="br-tab-btn-reporte">📊 Reporte</button>
        <button class="tab-pill" id="br-tab-btn-hallazgos">⚠️ Hallazgos</button>
        <button class="tab-pill" id="br-tab-btn-asign" style="display:none">👥 Asignaciones</button>
      </div>

      <!-- TAB CONTEO -->
      <div id="br-tab-conteo">
        <!-- Selector de Tarima + Fecha de trabajo -->
        <div class="sel-wrap" style="flex-wrap:wrap;gap:8px;align-items:center">
          <label style="font-size:.8rem;color:#666;font-weight:600">Tarima (BoomRoom):</label>
          <select id="br-sel-tarima" style="padding:7px 10px;border:1px solid #ccc;border-radius:8px;font-size:.88rem;min-width:180px"></select>
          <div style="position:relative;display:inline-block">
            <input type="text" id="br-tarima-buscar" autocomplete="off"
              placeholder="🔍 Buscar tarima (ej: BR001.01)"
              style="padding:7px 10px;border:1px solid #ccc;border-radius:8px;font-size:.88rem;min-width:200px">
            <div id="br-tarima-dropdown" style="display:none;position:absolute;top:100%;left:0;z-index:50;background:#fff;border:1px solid #ccc;border-radius:8px;max-height:220px;overflow-y:auto;min-width:220px;box-shadow:0 4px 12px rgba(0,0,0,.15);margin-top:2px"></div>
          </div>
          <label style="font-size:.8rem;color:#666;font-weight:600;margin-left:12px">F. Trabajo:</label>
          <input type="date" id="br-fecha-trabajo" style="padding:7px 10px;border:1px solid #ccc;border-radius:8px;font-size:.88rem;min-height:36px">
          <span class="prog" id="br-prog-text" style="font-size:.78rem;color:#888;margin-left:8px"></span>
        </div>

        <!-- Buscador SKU/nombre — mismo patrón que Embarques (sku-search) -->
        <div style="margin-bottom:.75rem;display:flex;align-items:center;gap:8px;flex-wrap:wrap">
          <input id="br-sku-search" type="search" placeholder="🔍 Buscar SKU o nombre..."
            style="flex:1;max-width:380px;padding:9px 14px;font-size:.9rem;border:1px solid #ccc;border-radius:8px;min-height:42px;-webkit-appearance:none;background:#fff">
          <span id="br-sku-search-count" style="font-size:.78rem;color:#888"></span>
        </div>

        <!-- Formulario agregar SKU — siempre visible cuando hay tarima seleccionada -->
        <div id="br-add-form" style="display:none;background:#fafaf8;border:1px solid #e0e0e0;border-radius:10px;padding:.75rem .9rem;margin-bottom:.75rem;margin-top:.5rem">
          <div style="font-size:.78rem;font-weight:600;color:#555;margin-bottom:.5rem;text-transform:uppercase;letter-spacing:.3px">➕ Agregar SKU no teórico</div>
          <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:flex-end">
            <div>
              <label style="font-size:.74rem;color:#666;display:block;margin-bottom:3px">SKU</label>
              <input type="text" id="br-add-sku" placeholder="SKU" autocomplete="off" autocorrect="off" style="padding:7px 10px;border:1px solid #ccc;border-radius:6px;width:110px;font-size:.88rem">
            </div>
            <div style="flex:1;min-width:160px">
              <label style="font-size:.74rem;color:#666;display:block;margin-bottom:3px">Descripción <span id="br-add-desc-auto" style="color:#1e7e34;font-size:.7rem"></span></label>
              <input type="text" id="br-add-desc" placeholder="Se completa automático" style="padding:7px 10px;border:1px solid #ccc;border-radius:6px;width:100%;font-size:.88rem;background:#f5f5f3" readonly>
            </div>
            <div>
              <label style="font-size:.74rem;color:#666;display:block;margin-bottom:3px">Cantidad física</label>
              <input type="number" id="br-add-qty" min="0" placeholder="0" style="padding:7px 10px;border:1px solid #ccc;border-radius:6px;width:90px;font-size:.88rem">
            </div>
            <button class="btn btn-dark" id="br-btn-add-sku" style="min-width:100px;padding:8px 14px">+ Agregar</button>
          </div>
          <div id="br-add-msg" style="font-size:.82rem;margin-top:.4rem"></div>
        </div>

        <!-- Tabla conteo BoomRoom -->
        <div class="tbl-wrap">
          <table>
            <thead><tr>
              <th>SKU</th>
            ...
            <tbody id="br-tbody-conteo"></tbody>
          </table>
        </div>
        <div class="actions">
          <button class="btn btn-orange" id="br-btn-hallazgo">⚠ Crear hallazgo</button>
          <button class="btn btn-dark" id="br-btn-save">Guardar conteo ✓</button>
        </div>
      </div>

      <!-- TAB REPORTE -->
      <div id="br-tab-reporte" style="display:none">
        <div style="background:#fff;border:1px solid #e0e0e0;border-radius:10px;padding:.875rem 1rem;margin-bottom:1rem">
          <button class="btn btn-green" id="br-btn-export">⬇ Exportar Reporte BoomRoom</button>
        </div>
        <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;margin-bottom:.875rem">
          <label style="font-size:.78rem;color:#666">Clasif:</label>
          <div style="display:inline-flex;gap:2px;background:#f0f0ee;padding:3px;border-radius:8px">
            <button class="br-clasif-btn" data-clasif="todos" style="font-size:.72rem;padding:5px 10px;border:none;border-radius:6px;cursor:pointer;background:#1a1a1a;color:#fff;font-weight:600;font-family:inherit">Todos</button>
            <button class="br-clasif-btn" data-clasif="Auditado" style="font-size:.72rem;padding:5px 10px;border:none;border-radius:6px;cursor:pointer;background:transparent;color:#666;font-weight:600;font-family:inherit">Auditado</button>
            <button class="br-clasif-btn" data-clasif="Pendiente" style="font-size:.72rem;padding:5px 10px;border:none;border-radius:6px;cursor:pointer;background:transparent;color:#666;font-weight:600;font-family:inherit">Pendiente</button>
          </div>
          <input type="date" id="br-filtro-desde" style="padding:6px 10px;font-size:.82rem;border:1px solid #ccc;border-radius:6px">
          <input type="date" id="br-filtro-hasta" style="padding:6px 10px;font-size:.82rem;border:1px solid #ccc;border-radius:6px">
          <button id="br-filtro-clear" style="font-size:.72rem;background:none;border:1px solid #ccc;border-radius:6px;padding:4px 10px;cursor:pointer">× Limpiar</button>
        </div>
        <div class="tbl-wrap">
          <table><thead><tr>
            <th>Tarima (BoomRoom)</th>
            <th class="tc">Ítems teórico</th><th class="tc">Ítems finales</th>
            <th class="tc">Exactos</th><th class="tc">Sobrantes</th><th class="tc">Faltantes</th>
            <th class="tc">% Cobertura</th><th class="tc">IRA</th>
            <th class="tc">Costo total Q</th><th>Fecha trabajo</th><th>Responsable</th><th>Clasificación</th>
          </tr></thead><tbody id="br-tbody-reporte"></tbody></table>
        </div>
      </div>

      <!-- TAB HALLAZGOS — igual que Embarques -->
      <div id="br-tab-hallazgos" style="display:none">
        <div id="br-hall-list"></div>
        <p class="empty" id="br-hall-empty">Sin hallazgos registrados aún.</p>
      </div>

      <!-- TAB ASIGNACIONES — igual que Embarques, solo supervisores -->
      <div id="br-tab-asign" style="display:none">
        <div id="br-asign-content"></div>
      </div>
...
function setupMic(btnId, langSelId, statusId, targetTextareaId){
  ...
  if(!SpeechRec) {
    btn.disabled = true;
    btn.title = 'Para dictar usá Google Chrome. Tu navegador actual no soporta dictado por voz.';
    btn.style.opacity = '0.5';
    // Show a tap-to-learn tooltip on mobile
    btn.addEventListener('click', function(){
      alert('⚠️ Dictado no disponible\n\nEste dispositivo necesita Google Chrome para dictar.\n\nAbrí esta app en Chrome e intentá de nuevo.');
    });
    return false;
  }
  var rec = null;
  ...
  btn.addEventListener('click', function(e){
    ...
    rec.onerror = function(ev){
      ...
      if(ev.error === 'not-allowed' || ev.error === 'permission-denied') {
        ...
        alert('🎤 Permiso de micrófono denegado\n\nPasos para activarlo:\n1. Tocá el ícono 🔒 o ⚠️ en la barra de Chrome\n2. Buscá "Micrófono" y cambialo a Permitir\n3. Recargá la página e intentá de nuevo');
      } else if(ev.error === 'network') {
        if(statusEl) statusEl.textContent = 'Sin conexión para dictar';
      } else if(ev.error === 'no-speech') {
        if(statusEl) statusEl.textContent = 'No se detectó voz';
      } else {
...
// ── abrirFotoSheet ────────────────────────────────────────────────────────────
// Muestra un bottom-sheet para elegir entre cámara y galería en Android
// (teléfonos Y tablets — incluyendo Samsung Galaxy Tab con Chrome/Edge).
// En iOS el picker nativo ya ofrece ambas opciones, así que va directo.
// callback(file) se llama con el File seleccionado.
//
// FIX (jun-2026, Samsung Galaxy Tab S10 FE+): usar getUserMedia para la cámara
// en lugar de input[capture], que en tablets Samsung no muestra opción de cámara.
function abrirFotoSheet(callback) {
  var isAndroid = /android/i.test(navigator.userAgent);

  // iOS y desktop: ir directo al picker nativo (ya incluye cámara en iOS)
  if (!isAndroid) {
    var inp = document.createElement('input');
    inp.type = 'file'; inp.accept = 'image/*';
    inp.onchange = function(){ if(inp.files && inp.files[0]) callback(inp.files[0]); };
    inp.click();
    return;
  }

  // ── Android: mostrar sheet con cámara via getUserMedia ───────────────────
  var overlay = document.createElement('div');
  overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,.48);z-index:99999;display:flex;align-items:flex-end;justify-content:center';

  var sheet = document.createElement('div');
  sheet.style.cssText = 'background:#fff;border-radius:18px 18px 0 0;padding:18px 16px 36px;width:100%;max-width:500px;display:flex;flex-direction:column;gap:10px;box-shadow:0 -4px 24px rgba(0,0,0,.18)';
  sheet.innerHTML =
    '<div style="width:40px;height:4px;background:#e0e0e0;border-radius:4px;margin:0 auto 8px"></div>'
    +'<div style="font-size:.9rem;color:#555;text-align:center;font-weight:600;margin-bottom:2px">Agregar foto</div>'
    +'<button id="_fsheet_cam" style="padding:14px 12px;font-size:1rem;border:none;background:#f5f5f3;border-radius:12px;cursor:pointer;text-align:left">📷&nbsp;&nbsp;Tomar foto con la cámara</button>'
    +'<button id="_fsheet_gal" style="padding:14px 12px;font-size:1rem;border:none;background:#f5f5f3;border-radius:12px;cursor:pointer;text-align:left">🖼️&nbsp;&nbsp;Elegir de la galería</button>'
    +'<button id="_fsheet_cancel" style="padding:10px;font-size:.88rem;border:none;background:none;color:#999;cursor:pointer;margin-top:2px">Cancelar</button>';
  overlay.appendChild(sheet);
  document.body.appendChild(overlay);

  // Input para galería (sin capture)
  var inpGal = document.createElement('input');
  inpGal.type = 'file'; inpGal.accept = 'image/*';
  inpGal.style.cssText = 'position:fixed;top:-9999px;left:-9999px;width:1px;height:1px;opacity:0;pointer-events:none';
  document.body.appendChild(inpGal);

  function cleanup(){
    if(overlay.parentNode) document.body.removeChild(overlay);
    if(inpGal.parentNode) document.body.removeChild(inpGal);
  }

  inpGal.onchange = function(){
    cleanup();
    if(inpGal.files && inpGal.files[0]) callback(inpGal.files[0]);
  };

  // ── Botón Galería ────────────────────────────────────────────────────────
  sheet.querySelector('#_fsheet_gal').onclick = function(){
    if(overlay.parentNode) document.body.removeChild(overlay);
    inpGal.click();
  };

  // ── Botón Cámara: usar getUserMedia directamente ─────────────────────────
  sheet.querySelector('#_fsheet_cam').onclick = function(){
    if(overlay.parentNode) document.body.removeChild(overlay);
    abrirCamaraGetUserMedia(callback);
  };

  sheet.querySelector('#_fsheet_cancel').onclick = cleanup;
  overlay.addEventListener('click', function(e){ if(e.target === overlay) cleanup(); });
}

// ── abrirCamaraGetUserMedia ───────────────────────────────────────────────
// Abre la cámara trasera usando getUserMedia y muestra un visor en pantalla.
// Al tocar "Capturar" toma la foto y la convierte a File para el callback.
// Funciona en Samsung Galaxy Tab donde input[capture] no muestra cámara.
function abrirCamaraGetUserMedia(callback) {
  if(!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
    // Fallback: abrir selector de archivos normal
    var inp = document.createElement('input');
    inp.type = 'file'; inp.accept = 'image/*';
    inp.onchange = function(){ if(inp.files && inp.files[0]) callback(inp.files[0]); };
    inp.click();
  ...
  // UI del visor de cámara
  var camOverlay = document.createElement('div');
  camOverlay.style.cssText = 'position:fixed;inset:0;background:#000;z-index:999999;display:flex;flex-direction:column;align-items:center;justify-content:center';
  camOverlay.innerHTML =
    '<video id="_cam_video" autoplay playsinline style="width:100%;max-height:75vh;object-fit:cover"></video>'
    +'<div style="display:flex;gap:16px;margin-top:16px;padding:0 16px;width:100%;justify-content:center">'
    +'<button id="_cam_cancel" style="flex:1;max-width:140px;padding:14px;font-size:1rem;border:none;background:#333;color:#fff;border-radius:12px;cursor:pointer">✕ Cancelar</button>'
    +'<button id="_cam_capture" style="flex:1;max-width:140px;padding:14px;font-size:1rem;border:none;background:#1e7e34;color:#fff;border-radius:12px;cursor:pointer;font-weight:700">📸 Capturar</button>'
    +'</div>'
    +'<canvas id="_cam_canvas" style="display:none"></canvas>';
  document.body.appendChild(camOverlay);

  var video   = document.getElementById('_cam_video');
  var canvas  = document.getElementById('_cam_canvas');
  var stream  = null;

  function stopCamera(){
    if(stream) stream.getTracks().forEach(function(t){ t.stop(); });
    if(camOverlay.parentNode) document.body.removeChild(camOverlay);
  }

  // Pedir acceso a cámara trasera
  navigator.mediaDevices.getUserMedia({
    ...
    stopCamera();
    // Fallback a selector de archivos si no hay permiso
    alert('No se pudo acceder a la cámara. Usá la galería o revisá los permisos de cámara en Chrome.');
    var inp = document.createElement('input');
    inp.type = 'file'; inp.accept = 'image/*';
    inp.onchange = function(){ if(inp.files && inp.files[0]) callback(inp.files[0]); };
    inp.click();
  ...
  // Capturar foto
  document.getElementById('_cam_capture').onclick = function(){
    if(!video.videoWidth){ alert('La cámara aún no está lista. Esperá un momento.'); return; }
  ...
  document.getElementById('_cam_cancel').onclick = stopCamera;
}
...
function renderLoginNameOptions(){
  ...
  if(window.ACCESS_SCOPE === 'rgis'){
    html += '<optgroup label="RGIS NODUS">';
    html += '<option>Operador 1</option><option>Operador 2</option>';
    html += '<option>Operador 3</option><option>Operador 4</option>';
    html += '<option>Operador 5</option><option>Operador 6</option>';
    html += '<option>Operador 7</option><option>Operador 8</option>';
    html += '</optgroup>';
    html += '<optgroup label="RGIS CDG">';
    html += '<option>Operador 9</option><option>Operador 10</option>';
    html += '<option>Operador 11</option><option>Operador 12</option>';
    html += '<option>Operador 13</option><option>Operador 14</option>';
    html += '</optgroup>';
    ...
    html += '</optgroup>';
    // FIX (vie 12-jun-2026, v5.2.31): bodega users solo con clave bodega (como RGIS con su clave)
    // Removido: optgroup Bodega CDG del scope full — usan su propia clave
  }
  sel.innerHTML = html;
  sel.value = '';
}
...
function fetchAccessKey(){
  ...
  .then(function(rows){
    if(!rows || !rows.length) return { accessKey:'', bodegaKey:'12345', rgisKey:'', cemacoKey:'cemaco2026', gateVersion:'v1' };
    var v = rows[0].value;
    try {
      ...
      return {
        ...
        // FIX (ago-2026): clave de acceso para el grupo Cemaco, mismo mecanismo
        // que accessKey/bodegaKey/rgisKey. Default 'cemaco2026' mientras no se
        // agregue el campo "cemacoKey" al JSON de app_config en Supabase.
        cemacoKey:   (cfg && cfg.cemacoKey)   ? String(cfg.cemacoKey)   : 'cemaco2026',
        gateVersion: (cfg && cfg.gateVersion) ? String(cfg.gateVersion) : 'v1'
      };
    }
    catch(e){ return { accessKey:'', bodegaKey:'12345', rgisKey:'', cemacoKey:'cemaco2026', gateVersion:'v1' }; }
  });
}
...
/* ── GATE CEMACO — mismo mecanismo que el gate de HPC, clave propia ── */
var CEMACO_GATE_MARK_KEY = 'cemaco_gate_ok_date';

function cemacoGatePassedToday(){
  try {
    var today = new Date().toLocaleDateString('en-CA', { timeZone:'America/Guatemala' });
    return sessionStorage.getItem(CEMACO_GATE_MARK_KEY) === today;
  } catch(e){ return false; }
}
function markCemacoGatePassed(){
  try {
    var today = new Date().toLocaleDateString('en-CA', { timeZone:'America/Guatemala' });
    sessionStorage.setItem(CEMACO_GATE_MARK_KEY, today);
  } catch(e){ /* si sessionStorage falla, simplemente pedirá la clave de nuevo */ }
}

function showCemacoEmpty(){
  document.getElementById('pg-gate-cemaco').style.display = 'none';
  document.getElementById('pg-cemaco').style.display = 'flex';
}

function tryGateCemaco(){
  var inp = document.getElementById('gate-cemaco-pass');
  var err = document.getElementById('gate-cemaco-err');
  var val = (inp.value || '').trim();
  if(!val){ err.textContent='Ingresa la clave.'; err.style.display='block'; return; }
  err.style.display='none';
  var loading = document.getElementById('gate-cemaco-loading');
  if(loading) loading.style.display='block';
  fetchAccessKey().then(function(cfg){
    if(loading) loading.style.display='none';
    var key = cfg.cemacoKey || 'cemaco2026';
    if(val === key){ markCemacoGatePassed(); showCemacoEmpty(); }
    else { err.textContent='Clave incorrecta.'; err.style.display='block'; inp.value=''; inp.focus(); }
  }).catch(function(){
    if(loading) loading.style.display='none';
    err.textContent='Error al verificar. Reintentá.'; err.style.display='block';
  });
}
document.getElementById('btn-gate-cemaco-enter').addEventListener('click', tryGateCemaco);
document.getElementById('gate-cemaco-pass').addEventListener('keydown', function(e){ if(e.key==='Enter') tryGateCemaco(); });
document.getElementById('btn-gate-cemaco-volver').addEventListener('click', function(){
  document.getElementById('pg-gate-cemaco').style.display = 'none';
  document.getElementById('gate-cemaco-pass').value = '';
  document.getElementById('gate-cemaco-err').style.display = 'none';
  document.getElementById('pg-grupo').style.display = 'flex';
});
...
document.getElementById('btn-login-enter').addEventListener('click',function(){
  ...
  if(window.ACCESS_SCOPE === 'rgis'){
    ...
    // FIX (sab 13-jun-2026): RGIS requiere nombre real para trazabilidad
    currentUser = { name:n, role:'rgis', operadorGenerico:n }; // temporal
    window._rgisOperadorGenerico = n; // guardar operador genérico
    // Ocultar pg-login y mostrar pantalla de nombre real
    document.getElementById('pg-login').style.display = 'none';
    var prEl = document.getElementById('pg-rgis-nombre');
    if(prEl){
      prEl.style.display = 'flex';
      var opLabel = prEl.querySelector('#rgis-op-label');
      if(opLabel) opLabel.textContent = n;
      var nameInp = prEl.querySelector('#rgis-nombre-real');
      if(nameInp){ nameInp.value = ''; setTimeout(function(){ nameInp.focus(); }, 100); }
      var errEl = prEl.querySelector('#rgis-nombre-err');
      if(errEl) errEl.style.display = 'none';
    }
    return; // afterLogin se llama desde pg-rgis-nombre
...
document.getElementById('btn-logout').addEventListener('click',function(){
  ...
  if(typeof window.bodCleanup === 'function') window.bodCleanup();
  // FIX (sab 13-jun-2026): limpiar variables RGIS al salir
  window._rgisOperadorGenerico = null;
  var _rgisScreen = document.getElementById('pg-rgis-nombre');
  if(_rgisScreen) _rgisScreen.style.display = 'none';
  lastVersion = -1;
  window.serverLocks = {};
  window.ACCESS_SCOPE = 'full';
  try { sessionStorage.removeItem(GATE_MARK_KEY); sessionStorage.removeItem(GATE_SCOPE_KEY); } catch(e){}
  currentUser=null;
  window._cdgv2Joined = {};  // FIX (jue 19-jun-2026, BW): reset unirse guard on logout
...
function afterLogin(){
  ...
  if(currentUser.role === 'rgis'){
    // FIX (sab 13-jun-2026): RGIS accede a Hamilton + CDG desde el menu principal
    var _mcBod = document.getElementById('mc-bodega');
    if(_mcBod) _mcBod.style.display = 'none'; // RGIS no ve Bodega CDG
    navTo('pg-menu','📦 Conteo de Inventario');
    var _mgc = document.getElementById('menu-grid-cards'); // revelar cards
...
function updateSubmenúCounts(){
  ...
  var br=conts.filter(function(c){return teorico[c]&&teorico[c].type==='BoomRoom';}).length;
  ...
  var cntBr=document.getElementById('cnt-boomroom');
  if(cntBr) cntBr.textContent=br;
  // Refresh BoomRoom si está activa
  if(typeof window._brRefreshIfActive==='function') window._brRefreshIfActive();
}
...
document.getElementById('sm-boomroom').addEventListener('click',function(){
  navTo('pg-boomroom','🏗 BoomRooms');
  brInit();
});
...
// RGIS: Wire pantalla nombre real (trazabilidad) — sab 13-jun-2026
(function(){
  var btn  = document.getElementById('rgis-nombre-btn');
  var inp  = document.getElementById('rgis-nombre-real');
  var err  = document.getElementById('rgis-nombre-err');
  var back = document.getElementById('rgis-nombre-volver');
  function confirmar(){
    var nombre = (inp ? inp.value : '').trim();
    if(!nombre){ if(err) err.style.display='block'; if(inp) inp.focus(); return; }
    if(err) err.style.display='none';
    if(window.currentUser){
      window.currentUser.name            = nombre;
      window.currentUser.nombreReal       = nombre;
      window.currentUser.operadorGenerico = window._rgisOperadorGenerico||'';
    }
    var prEl = document.getElementById('pg-rgis-nombre');
    if(prEl) prEl.style.display = 'none';
    // Mostrar main-wrap y user badge (normalmente lo hace el handler del botón Entrar)
    document.getElementById('pg-login').style.display = 'none';
    document.getElementById('main-wrap').style.display = '';
  ...
  if(inp)  inp.addEventListener('keydown', function(e){ if(e.key==='Enter') confirmar(); });
  if(back) back.addEventListener('click', function(){
    var prEl = document.getElementById('pg-rgis-nombre');
    if(prEl) prEl.style.display = 'none';
    window.currentUser = null;
    window._rgisOperadorGenerico = null;
    var pgL = document.getElementById('pg-login'); if(pgL) pgL.style.display='flex';
  });
})();

// FIX (ago-2026): initGate() ya NO corre automáticamente al cargar la página.
// Ahora la primera pantalla es #pg-grupo (HPC / Cemaco). Solo al elegir HPC
// se revela el gate y se dispara initGate() (que decide si pide la clave o
// si la salta directo al login, exactamente igual que antes).
document.getElementById('mc-grupo-hpc').addEventListener('click', function(){
  document.getElementById('pg-grupo').style.display = 'none';
  document.getElementById('pg-gate').style.display = 'flex';
  initGate();
});
document.getElementById('mc-grupo-cemaco').addEventListener('click', function(){
  document.getElementById('pg-grupo').style.display = 'none';
  if(cemacoGatePassedToday()){
    showCemacoEmpty();
  } else {
    document.getElementById('pg-gate-cemaco').style.display = 'flex';
  }
});
document.getElementById('btn-cemaco-volver').addEventListener('click', function(){
  document.getElementById('pg-cemaco').style.display = 'none';
  document.getElementById('pg-grupo').style.display = 'flex';
});
...
document.getElementById('btn-hall-save').addEventListener('click',async function(){
  ...
  if(editingId) {
    ...
    // FIX (jul-2026): BoomRoom override — usar brTarima si viene desde BoomRoom
    var hallCont = window._hallContOverride || current;
    window._hallContOverride = null;
    hall = {
      id:makeId(), cont:hallCont, quePaso:desc,
  ...
  renderHallazgosList();
  // FIX (jul-2026): si el hallazgo se creó/editó desde BoomRoom (cont = tarima BR),
  // también hay que repintar su propio listado (#br-hall-list), que no se actualiza
  // solo con renderHallazgosList() (esa función pinta #hall-list de Embarques,
  // ahora ya filtrado para excluir las tarimas BR).
  if (typeof window.brRenderHallazgos === 'function') window.brRenderHallazgos();
...
function renderHallazgosList(){
  ...
  // FIX (jul-2026): excluir también hallazgos cuyo cont es una tarima
  // BoomRoom (h.cont pertenece a brConts()). Antes aparecían acá porque
  // este filtro solo miraba `tipo`, y los hallazgos de BoomRoom no traen
  // tipo — se guardan con cont=brTarima pero sin marca de tipo alguna.
  var brContList = (typeof window.brConts === 'function') ? window.brConts() : [];
  var visibleHall = hallazgos.filter(function(h){
    return h.tipo !== 'CDG' && brContList.indexOf(h.cont) < 0; // vista Embarques: excluir los de BoomRoom
  });
...
function startPolling() {
  ...
  pollInterval = setInterval(pollNow, 15000); // BW-FIX: 5000→15000ms, -67% bandwidth
}
...
// FIX (jul-2026): mismo buscador pero para la pestaña Conteo de BoomRoom.
// Réplica exacta de applySkuSearchFilter, apuntando a #br-tbody-conteo.
// Se expone global para poder re-aplicarse al final de cada brRenderConteo
// (igual que applySkuSearchFilter se re-aplica al final de renderConteo),
// así el filtro no se "pierde" cuando la tabla se repinta por un poll.
window.applyBrSkuSearchFilter = function() {
  var inp = document.getElementById('br-sku-search');
  if(!inp) return;
  var q = (inp.value || '').trim().toLowerCase();
  var rows = document.querySelectorAll('#br-tbody-conteo tr');
  var shown = 0;
  rows.forEach(function(row) {
    if(!row.cells || row.cells.length < 2) return;
    var sku  = (row.cells[0] ? row.cells[0].textContent : '').toLowerCase();
  ...
  var cnt = document.getElementById('br-sku-search-count');
  if(cnt) cnt.textContent = q ? (shown + ' resultado' + (shown!==1?'s':'')) : '';
};

document.getElementById('br-sku-search').addEventListener('input', window.applyBrSkuSearchFilter);

// Limpiar el buscador al cambiar de tarima
document.getElementById('br-sel-tarima').addEventListener('change', function() {
  var s = document.getElementById('br-sku-search');
  if(s) s.value = '';
  var c = document.getElementById('br-sku-search-count');
  if(c) c.textContent = '';
});
</script>
...
var CDGV2_LISTA_POLL_MS = 20000;  // BW-FIX: 5000→20000ms, -75% CDG lista poll
...
var wmsAuditadoMap    = {};    // SKU (upper) → 'Auditado'|'No Auditado'|'' para la comparación
...
function wmsCargarEstado(licId){
  ...
  Promise.all([pWms, pLineas]).then(function(results){
    ...
    if(fuente === 'colaborativo') {
      ...
      // Construir mapa SKU → estado auditado desde meta.auditadoManual
      var audMeta = (licResp.ok && licResp.meta && licResp.meta.auditadoManual) ? licResp.meta.auditadoManual : {};
      wmsAuditadoMap = {};
      wmsLineasCDG.forEach(function(l){
        var skuUp = String(l.sku||'').trim().toUpperCase();
        if(!skuUp) return;
        var estado = audMeta[l.id] || '';
        // Si hay cualquier estado establecido para ese SKU, lo asignamos.
        // Prioridad: 'No Auditado' > 'Auditado' > '' (si hay discrepancia entre líneas del mismo SKU)
        if(!wmsAuditadoMap[skuUp]) {
          wmsAuditadoMap[skuUp] = estado;
        } else if(estado === 'No Auditado') {
          wmsAuditadoMap[skuUp] = 'No Auditado';
        } else if(estado === 'Auditado' && wmsAuditadoMap[skuUp] === '') {
          wmsAuditadoMap[skuUp] = 'Auditado';
        }
      });
    } else {
...
function wmsRenderizarTabla(){
  ...
  var h = '<table><thead><tr>'
    ...
    +'<th class="tc">Diferencia</th>'
    +'<th class="tc" style="min-width:110px">Estado</th>'
    +'</tr></thead><tbody>';
  filas.forEach(function(f){
    ...
    var audVal  = wmsAuditadoMap[String(f.sku||'').toUpperCase()] || '';
    var audBg   = audVal==='Auditado' ? '#e8f5e9' : audVal==='No Auditado' ? '#fdecea' : '#f5f5f3';
    var audFg   = audVal==='Auditado' ? '#2e7d32' : audVal==='No Auditado' ? '#c0392b' : '#aaa';
    var audHtml = audVal
      ? '<span style="font-size:.72rem;padding:2px 8px;border-radius:5px;font-weight:600;background:'+audBg+';color:'+audFg+'">'+escH(audVal)+'</span>'
...
function wmsExportarManifiesto(){
  ...
  var colHdr = ['CODIGO', 'DESCRIPCION', 'CANTIDAD', 'VALIDADO EN CDG', 'DIFERENCIA', 'ESTADO'];
  var dataRows = wmsFilasComp.map(function(f){
    var audVal = wmsAuditadoMap[String(f.sku||'').toUpperCase()] || '';
    return [f.sku, f.descripcion || 'sin descripción registrada', f.cantWMS, f.cantCDG, f.diferencia, audVal || '—'];
  });
...
function renderCdgv2Lista(licencias){
  ...
  var html = '<div class="tbl-wrap"><table style="width:100%">'
    +'<thead><tr>'
    +'<th style="text-align:left;padding:8px 10px;font-size:.78rem;color:#555;border-bottom:2px solid #e0e0e0;white-space:nowrap">Correlativo</th>'
    +'<th style="text-align:left;padding:8px 10px;font-size:.78rem;color:#555;border-bottom:2px solid #e0e0e0;white-space:nowrap">Tipo</th>'
    +'<th style="text-align:left;padding:8px 10px;font-size:.78rem;color:#555;border-bottom:2px solid #e0e0e0;white-space:nowrap">Creado por</th>'
    +'<th style="text-align:left;padding:8px 10px;font-size:.78rem;color:#555;border-bottom:2px solid #e0e0e0;white-space:nowrap">Fecha</th>'
    +'<th class="tc" style="padding:8px 10px;font-size:.78rem;color:#555;border-bottom:2px solid #e0e0e0;white-space:nowrap">Líneas</th>'
    +'<th style="text-align:left;padding:8px 10px;font-size:.78rem;color:#555;border-bottom:2px solid #e0e0e0">Usuarios</th>'
    +'<th class="tc" style="padding:8px 10px;font-size:.78rem;color:#555;border-bottom:2px solid #e0e0e0;white-space:nowrap">Estado</th>'
    +'<th class="tc" style="padding:8px 10px;font-size:.78rem;color:#555;border-bottom:2px solid #e0e0e0;white-space:nowrap">Acción</th>'
    +'</tr></thead><tbody>';

  unicas.forEach(function(l, idx){
    ...
    var rowBg       = idx % 2 === 0 ? '#fff' : '#fafaf8';
    // Formatear fecha de creación (solo fecha, sin hora)
    var fechaFmt = '—';
    if(l.fechaCreacion) {
      try {
        var d = new Date(l.fechaCreacion);
        fechaFmt = d.toLocaleDateString('es-GT', { day:'2-digit', month:'2-digit', year:'2-digit' });
      } catch(e){}
    }
    // Solo creador o supervisor puede eliminar
    var esSup = typeof isSup === 'function' && isSup();
    var esCreadorLista = currentUser && l.creadoPor === currentUser.name;
    var btnEliminar = (esSup || esCreadorLista)
      ? '<button class="cdgv2-lista-del-btn" data-id="'+escH(l.id)+'" data-lineas="'+l.totalLineas+'"'
        +' style="background:#fdecea;color:#c0392b;font-size:.72rem;padding:3px 8px;border-radius:5px;'
        +'border:1px solid #f5c6cb;cursor:pointer;white-space:nowrap" title="Eliminar licencia">🗑 Borrar</button>'
      : '';
    html += '<tr class="cdgv2-lic-item" data-id="'+escH(l.id)+'" style="background:'+rowBg+';cursor:pointer" '
      +'onmouseover="this.style.background=\'#f0f4ff\'" onmouseout="this.style.background=\''+rowBg+'\'">'
      +'<td style="padding:10px 10px;font-weight:700;font-size:.9rem;white-space:nowrap">'+escH(l.id)+'</td>'
      +'<td style="padding:10px 10px;font-size:.82rem;color:#555;white-space:nowrap">'+escH(l.tipo)+'</td>'
      +'<td style="padding:10px 10px;font-size:.82rem;color:#666;white-space:nowrap">'+escH(l.creadoPor)+'</td>'
      +'<td style="padding:10px 10px;font-size:.78rem;color:#888;white-space:nowrap">'+escH(fechaFmt)+'</td>'
      +'<td class="tc" style="padding:10px 10px;font-size:.88rem;font-weight:600">'+l.totalLineas+'</td>'
      +'<td style="padding:10px 10px;font-size:.78rem;color:#888">'+escH(usuarios)+'</td>'
      +'<td class="tc" style="padding:10px 10px">'
        +'<span style="font-size:.72rem;padding:3px 10px;border-radius:10px;background:'+estadoColor+';color:'+estadoFg+';font-weight:600;white-space:nowrap">'+estadoText+'</span>'
      +'</td>'
      +'<td class="tc" style="padding:10px 10px">'+btnEliminar+'</td>'
      +'</tr>';
  });
  ...
  el.querySelectorAll('.cdgv2-lic-item').forEach(function(item){
    item.addEventListener('click', function(e){
      // No abrir licencia si se hizo clic en el botón de borrar
      if(e.target.classList.contains('cdgv2-lista-del-btn') || e.target.closest('.cdgv2-lista-del-btn')) return;
      cdgv2AbrirLicencia(item.dataset.id);
    });
  });

  el.querySelectorAll('.cdgv2-lista-del-btn').forEach(function(btn){
    btn.addEventListener('click', function(e){
      e.stopPropagation();
      var lid     = btn.dataset.id;
      var nLineas = parseInt(btn.dataset.lineas)||0;
      var aviso   = nLineas > 0
        ? '⚠️ La licencia "'+lid+'" tiene '+nLineas+' línea(s) capturadas.\n\n¿Seguro que querés eliminarla? Esta acción no se puede deshacer.'
        : '¿Eliminás la licencia "'+lid+'"? Esta acción no se puede deshacer.';
      if(!confirm(aviso)) return;
      if(!currentUser){ alert('Seleccioná tu nombre primero.'); return; }
      btn.textContent = 'Borrando…'; btn.disabled = true;
      cdgv2Fetch('DELETE', '/api/cdg/v2/'+lid, { usuario: currentUser.name })
        .then(function(d){
          if(!d.ok){ alert(d.error||'Error al eliminar'); btn.textContent='🗑 Borrar'; btn.disabled=false; return; }
          cdgv2CargarLista();
        }).catch(function(){ alert('Sin conexión'); btn.textContent='🗑 Borrar'; btn.disabled=false; });
    });
  });
}
...
function cdgv2AbrirLicencia(licenciaId){
  ...
  // FIX (jue 19-jun-2026, v5.2.33-BW): unirse solo 1 vez por licencia/sesión.
  // Evita writes Supabase repetidos cada vez que el estado recarga la página.
  if(!window._cdgv2Joined) window._cdgv2Joined = {};
  if(currentUser && !window._cdgv2Joined[licenciaId]) {
    window._cdgv2Joined[licenciaId] = true;
...
function renderCdgv2Header(meta){
  if(!meta) return;
  document.getElementById('cdgv2-lic-titulo').textContent = (meta.alias ? meta.alias + ' (' + meta.id + ')' : meta.id) + ' · ' + meta.tipo;
  ...
  // Botón editar licencia: visible para supervisores o el creador cuando activa
  var btnEditarLic = document.getElementById('btn-cdgv2-editar-lic');
  if(btnEditarLic) {
    var puedeEditar = meta.estado === 'activo' && (esSup || (currentUser && currentUser.name === meta.creadoPor));
    btnEditarLic.style.display = puedeEditar ? '' : 'none';
    if(!btnEditarLic._handlerOk) {
      btnEditarLic._handlerOk = true;
      btnEditarLic.addEventListener('click', function(){
        var form = document.getElementById('cdgv2-editar-form');
        if(!form) return;
        var meta2 = cdgv2LicenciaActual;
        if(meta2){
          document.getElementById('cdgv2-edit-nombre').value = meta2.alias || meta2.id || '';
          var tipoSel = document.getElementById('cdgv2-edit-tipo');
          if(tipoSel) tipoSel.value = meta2.tipo || 'CDG';
        }
        document.getElementById('cdgv2-editar-msg').innerHTML = '';
        form.style.display = form.style.display === 'none' ? '' : 'none';
      });
    }
  }
  var btnEditarCancel = document.getElementById('btn-cdgv2-editar-cancel');
  if(btnEditarCancel && !btnEditarCancel._handlerOk) {
    btnEditarCancel._handlerOk = true;
    btnEditarCancel.addEventListener('click', function(){
      document.getElementById('cdgv2-editar-form').style.display = 'none';
    });
  }
  var btnEditarOk = document.getElementById('btn-cdgv2-editar-ok');
  if(btnEditarOk && !btnEditarOk._handlerOk) {
    btnEditarOk._handlerOk = true;
    btnEditarOk.addEventListener('click', function(){
      if(!currentUser || !cdgv2LicenciaActual) return;
      var nuevoNombre = (document.getElementById('cdgv2-edit-nombre').value||'').trim();
      var nuevoTipo   = (document.getElementById('cdgv2-edit-tipo').value||'').trim();
      var msgEl = document.getElementById('cdgv2-editar-msg');
      if(!nuevoNombre){ msgEl.innerHTML = '<span style="color:#c0392b">Ingresá el correlativo.</span>'; return; }
      msgEl.innerHTML = '<span style="color:#888">Guardando…</span>';
      cdgv2Fetch('PATCH', '/api/cdg/v2/'+cdgv2LicenciaActual.id+'/meta',
        { usuario: currentUser.name, alias: nuevoNombre, tipo: nuevoTipo })
        .then(function(d){
          if(!d.ok){ msgEl.innerHTML = '<span style="color:#c0392b">'+escH(d.error||'Error')+'</span>'; return; }
          msgEl.innerHTML = '<span style="color:#1e7e34">✓ Guardado.</span>';
          document.getElementById('cdgv2-editar-form').style.display = 'none';
          cdgv2PollearLicencia(cdgv2LicenciaActual.id, false);
        }).catch(function(){ msgEl.innerHTML = '<span style="color:#c0392b">Sin conexión</span>'; });
    });
...
function renderCdgv2Lineas(){
  ...
  var h = '<div style="font-size:.78rem;font-weight:600;color:#555;margin-bottom:.5rem;'
    ...
    +'<th class="tc" style="min-width:60px">Tarima</th>'
    +'<th class="tc" style="min-width:80px">Evidencia</th>'
    +'<th style="min-width:110px">Usuario / Hora</th>'
    +'<th class="tc" style="min-width:100px">Auditado</th>'
  ...
  cdgv2Lineas.forEach(function(l){
    ...
    var tarimaMap2 = (cdgv2LicenciaActual && cdgv2LicenciaActual.tarimaMap) || {};
    var tarimaFmt  = tarimaMap2[l.id] != null ? String(tarimaMap2[l.id]) : '—';
    var audMap     = (cdgv2LicenciaActual && cdgv2LicenciaActual.auditadoManual) || {};
    var auditado   = audMap[l.id] || '';
    var audBg      = auditado === 'Auditado' ? '#e8f5e9' : auditado === 'No Auditado' ? '#fdecea' : '#f5f5f3';
    var audFg      = auditado === 'Auditado' ? '#2e7d32' : auditado === 'No Auditado' ? '#c0392b' : '#888';
    var audSelect  = !cerrada
      ? '<select class="cdgv2-auditado-sel" data-id="'+escH(l.id)+'"'
        +' style="font-size:.72rem;border:1px solid #ddd;border-radius:5px;padding:2px 4px;'
        +'background:'+audBg+';color:'+audFg+';cursor:pointer;font-weight:600">'
        +'<option value="">—</option>'
        +'<option value="Auditado"'+(auditado==='Auditado'?' selected':'')+'>Auditado</option>'
        +'<option value="No Auditado"'+(auditado==='No Auditado'?' selected':'')+'>No Auditado</option>'
        +'</select>'
      : '<span style="font-size:.72rem;padding:2px 7px;border-radius:5px;background:'+audBg+';color:'+audFg+';font-weight:600">'+(auditado||'—')+'</span>';
    ...
    h += '<tr'+rowCls+'>'
      ...
      +'<td class="tc" style="font-size:.85rem;font-weight:600;color:#555">'+escH(tarimaFmt)+'</td>'
  ...
  // Handler cambio auditado_manual (guardado en meta, sin alterar cdg_lineas)
  el.querySelectorAll('.cdgv2-auditado-sel').forEach(function(sel){
    sel.addEventListener('change', function(){
      var lid   = sel.dataset.id;
      var valor = sel.value;
      var lmeta = cdgv2LicenciaActual;
      if(!lmeta || !currentUser) return;
      // Actualizar mapa local de inmediato para que el siguiente render lo refleje
      if(!lmeta.auditadoManual) lmeta.auditadoManual = {};
      if(valor) lmeta.auditadoManual[lid] = valor;
      else      delete lmeta.auditadoManual[lid];
      // Actualizar color del select de inmediato
      var bg = valor==='Auditado'?'#e8f5e9':valor==='No Auditado'?'#fdecea':'#f5f5f3';
      var fg = valor==='Auditado'?'#2e7d32':valor==='No Auditado'?'#c0392b':'#888';
      sel.style.background = bg; sel.style.color = fg;
      // Persistir en meta (no en cdg_lineas — no requiere columna extra en Supabase)
      var patch = {};
      patch[lid] = valor;
      cdgv2Fetch('PATCH', '/api/cdg/v2/'+lmeta.id+'/meta',
        { usuario: currentUser.name, auditadoManual: patch })
        .then(function(d){
          if(!d.ok) alert(d.error||'Error al guardar estado auditado');
          // Sincronizar mapa con lo que devolvió el server
          if(d.auditadoManual) lmeta.auditadoManual = d.auditadoManual;
        }).catch(function(){ alert('Sin conexión'); });
    });
  });
...
function renderCdgv2Fotos(meta){
  ...
  grid.innerHTML = labels.map(function(lbl, i){
    var url = fotos[i] || '';
    if(url) {
      // Foto existente: mostrar miniatura + botón de reemplazar si no está cerrada
      var reemplazarBtn = !cerrada
        ? '<div style="position:absolute;bottom:0;left:0;right:0;background:rgba(0,0,0,.55);'
          +'text-align:center;padding:3px 0;font-size:.62rem;color:#fff;cursor:pointer" '
          +'class="cdg-foto-replace-overlay" data-idx="'+i+'">✏️ Cambiar</div>'
        : '';
      return '<div class="cdg-foto-slot has-photo" data-idx="'+i+'" style="position:relative">'
        +'<img src="'+escH(url)+'" alt="'+escH(lbl)+'"">'
        +'<div class="slot-label-overlay">'+escH(lbl)+'</div>'
        +reemplazarBtn
        +((!cerrada) ? '<input type="file" accept="image/*" style="display:none" class="cdg-foto-replace-inp" data-idx="'+i+'">' : '')
        +'</div>';
    }
    if(cerrada) {
    ...
    return '<div class="cdg-foto-slot" data-idx="'+i+'">'
      ...
      +'<input type="file" accept="image/*" style="position:absolute;inset:0;width:100%;height:100%;opacity:0;cursor:pointer;z-index:2" data-idx="'+i+'">'
      +'</div>';
  }).join('');

  if(cerrada) return;

  // FIX (jun-2026, Samsung Galaxy Tab S10 FE+): usar inputs directos en los slots
  // en lugar de abrirFotoSheet con callback. Chrome en tablets Samsung bloquea
  // el inp.click() cuando no está directamente en la cadena del gesto del usuario.
  // Solución: cada slot tiene su propio input[type=file] visible al tocar,
  // y mostramos el sheet ANTES de hacer click en el input correcto.
  grid.querySelectorAll('.cdg-foto-slot').forEach(function(slot){
    var inp = slot.querySelector('input[type=file]');

    function subirFotoEncabezado(file, esReemplazo, idx){
      ...
      window.uploadFotoAndGetUrl(file, 'cdg-gral', safeId, null)
        .then(function(fotoUrl){
          var endpoint = esReemplazo
            ? cdgv2Fetch('PATCH', '/api/cdg/v2/'+meta.id+'/fotos-encabezado-slot',
                { usuario: currentUser.name, idx: idx, foto: fotoUrl })
            : cdgv2Fetch('POST', '/api/cdg/v2/'+meta.id+'/fotos-encabezado',
    ...
    // Slots vacíos: click directo en el input (sin sheet intermedio)
    if(!slot.classList.contains('has-photo') && inp) {
      inp.addEventListener('change', function(){
        if(!this.files[0]) return;
        subirFotoEncabezado(this.files[0], false, parseInt(slot.dataset.idx));
      });
      slot.addEventListener('click', function(e){
        if(e.target === inp) return; // ya clickeó el input directamente
        inp.click();
      });
    }

    // Input de reemplazo (slots con foto existente)
    if(slot.classList.contains('has-photo') && inp) {
      inp.addEventListener('change', function(){
        if(!this.files[0]) return;
        subirFotoEncabezado(this.files[0], true, parseInt(this.dataset.idx));
      });
    }
  });

  // Click en overlay "Cambiar" → click directo en el input oculto del slot
  grid.querySelectorAll('.cdg-foto-replace-overlay').forEach(function(overlay){
    overlay.addEventListener('click', function(e){
      e.stopPropagation();
      var idx = parseInt(overlay.dataset.idx);
      var slot = grid.querySelector('.cdg-foto-slot[data-idx="'+idx+'"]');
      if(!slot) return;
      var inp2 = slot.querySelector('input[type=file]');
      if(inp2){ inp2.click(); return; }
      // Fallback si no hay input: usar abrirFotoSheet
      abrirFotoSheet(function(file){
        slot.innerHTML = '<div style="text-align:center;padding:.5rem;font-size:.75rem;color:#888">Subiendo…</div>';
        var safeId = cdgv2SafeRefId(meta.id);
        window.uploadFotoAndGetUrl(file, 'cdg-gral', safeId, null)
          .then(function(fotoUrl){
            return cdgv2Fetch('PATCH', '/api/cdg/v2/'+meta.id+'/fotos-encabezado-slot',
              { usuario: currentUser.name, idx: idx, foto: fotoUrl });
          })
          .then(function(d){
            if(!d.ok) alert(d.error||'Error al guardar foto');
...
// ── Detección de formato C999.###.## ────────────────────────────────────────
// C999.001.01 a C999.079.99 → "Multi código"
// C999.080.01 a C999.100.99 → "Big Pack Candis"
function cdgv2EsFormatoC999(sku) {
  return /^[Cc]999\.\d{3}\.\d{2}$/.test(String(sku||'').trim());
}
function cdgv2DescripcionC999(sku) {
  var m = String(sku).match(/^[Cc]999\.(\d{3})\.\d{2}$/);
  if(!m) return null;
  var grupo = parseInt(m[1], 10);
  if(grupo >= 1  && grupo <= 79)  return 'Multi código';
  if(grupo >= 80 && grupo <= 100) return 'Big Pack Candis';
  return null;
}
...
document.getElementById('cdgv2-add-sku').addEventListener('input', function(){
  ...
  // ── Detección C999.###.## ────────────────────────────────────────────────
  if(cdgv2EsFormatoC999(sku)) {
    var desc999 = cdgv2DescripcionC999(sku);
    cdgv2CerrarSugerencias();
    if(desc999) {
      autoEl.innerHTML     = '<span style="color:#1565c0">✓ '+desc999+'</span>';
      descEl.value         = desc999;
      descEl.readOnly      = true;
      descEl.style.background = '#f5f5f3';
      descEl.placeholder   = '';
      costoEl.value        = '0';
      ...
      autoEl.innerHTML     = '<span style="color:#e67e22">C999 fuera de rango — ingresá descripción manual</span>';
      descEl.value         = '';
      descEl.readOnly      = false;
      descEl.style.background = '#fff';
      descEl.placeholder   = 'Descripción';
      costoEl.value        = '0';
      cdgv2SkuValidado     = 'ok';
      cdgv2SkuValidadoPara = sku;
    }
...
document.getElementById('btn-cdgv2-add-linea').addEventListener('click', function(){
  ...
  var tarimaRaw = (document.getElementById('cdgv2-add-tarima').value||'').trim();
  var tarima    = tarimaRaw ? parseInt(tarimaRaw) : null;
  var msg  = document.getElementById('cdgv2-add-msg');
  if(!sku){ msg.innerHTML = '<span style=\"color:#c0392b\">Ingresá el SKU.</span>'; return; }
  // Formato C999.###.## siempre válido — no requiere catálogo
  var esC999 = typeof cdgv2EsFormatoC999 === 'function' && cdgv2EsFormatoC999(sku);
  if(!esC999) {
  ...
  // ── ANTI-DUPLICADO: deshabilitar el botón hasta que llegue la respuesta ──
  // Evita que toques múltiples o doble-toque en tablet envíen el POST 2+ veces.
  var addBtn = document.getElementById('btn-cdgv2-add-linea');
  ...
  cdgv2Fetch('POST', '/api/cdg/v2/'+meta.id+'/linea', {
    ...
    // Re-habilitar siempre al recibir respuesta
    addBtn.disabled = false;
    addBtn.textContent = '+ Agregar';
    if(!d.ok && !d.aviso){
      msg.innerHTML = '<span style=\"color:#c0392b\">'+escH(d.error||'Error')+'</span>'; return;
    }
    // Si se ingresó tarima, guardarla en el meta de la licencia (no en cdg_lineas)
    var lineaId = d.linea && d.linea.id;
    if(tarima != null && lineaId) {
      var tMap = {}; tMap[lineaId] = tarima;
      if(cdgv2LicenciaActual) {
        if(!cdgv2LicenciaActual.tarimaMap) cdgv2LicenciaActual.tarimaMap = {};
        cdgv2LicenciaActual.tarimaMap[lineaId] = tarima;
      }
      cdgv2Fetch('PATCH', '/api/cdg/v2/'+meta.id+'/meta',
        { usuario: currentUser.name, tarimaMap: tMap })
        .then(function(dm){ if(dm.tarimaMap && cdgv2LicenciaActual) cdgv2LicenciaActual.tarimaMap = dm.tarimaMap; })
        .catch(function(){});
    }
    ...
    document.getElementById('cdgv2-add-tarima').value = '';
    ...
    addBtn.disabled = false;
    addBtn.textContent = '+ Agregar';
    msg.innerHTML = '<span style=\"color:#c0392b\">Sin conexión. Reintentá.</span>';
  });
});
...
// ══════════════════════════════════════════════════════════════════════════
//  BODEGA CDG v3 — Reconstrucción desde cero
//  Fuente: Bloqque_Bodega_CDG.docx (jun-2026)
//  No usa /api/state. Endpoints: /api/bod/*
//  Tablas: bod_sesiones, bod_lineas, bod_teorico_952,
//          bod_manifiestos_control, bod_papel_trabajo,
//          bod_licencia_hija, sku_catalog
// ══════════════════════════════════════════════════════════════════════════

// ── Constantes ──────────────────────────────────────────────────────────
var BODEGA_LIST = ['Huvaldo Pérez','Ader Chávez','Angie Suar','Duma Pérez','Edgar Soto','Alejandra Galeros','Sharon Hernández'];
window.BODEGA_LIST = BODEGA_LIST;

// Prefijo de tarima por operador (A=Huvaldo, B=Ader, ...)
var BOD_PREFIJOS = {
  'Huvaldo Pérez':'A','Ader Chávez':'B','Angie Suar':'C','Duma Pérez':'D',
  'Edgar Soto':'E','Alejandra Galeros':'F','Sharon Hernández':'G'
};

var BOD_ENABLED_CLIENT = false;
window.BOD_ENABLED_CLIENT = BOD_ENABLED_CLIENT;

// RGIS_LIST también se expone aquí para que el login (script anterior) lo encuentre
// si por alguna razón el bloque de login corrió antes de que window.RGIS_LIST estuviera listo.
window.RGIS_LIST = window.RGIS_LIST || [
  'Operador 1','Operador 2','Operador 3','Operador 4','Operador 5',
  'Operador 6','Operador 7','Operador 8',
  'Operador 9','Operador 10','Operador 11','Operador 12','Operador 13','Operador 14'
];

// ── Estado interno ───────────────────────────────────────────────────────
var _bodCapturaSesion         = null;   // sesión activa en captura
var _bodAudSesion             = null;   // sesión activa en auditoría (tabs 1/2)
var _bodAudTab                = '4-control'; // tab activo en auditoría
var _bodAudPtActivo           = null;   // PT activo en papel de trabajo
var _bodTeorico               = {};     // mapa sku→cantidad_952 del teórico global
var _bodTeoricoGlobalCargado  = false;  // flag para no recargar teórico en cada visita
...
function bodEscA(s){ var d=document.createElement('div'); d.textContent=String(s||''); return d.innerHTML; }
...
function bodFmtFecha(iso){
  if(!iso) return '';
  var d = iso.slice(0,10).split('-');
  return d[2]+'/'+d[1]+'/'+d[0];
}

function bodCurrentUser(){ return window.currentUser ? window.currentUser.name : ''; }

// ── Prefijo tarima por operador ──────────────────────────────────────────
function bodPrefijo(nombre){
  return BOD_PREFIJOS[nombre] || 'X';
}

// ── Lookup SKU desde sku_catalog y bod_lineas ────────────────────────────
function bodLookupSku(q, sesId, cb){
  // q puede ser barra o sku
  var q2 = String(q||'').trim().toUpperCase();
  if(!q2){ cb(null); return; }
  // Intentar por barra primero
  bodFetch('GET','/api/bod/barra/'+encodeURIComponent(q2))
    .then(function(d){
      if(d && d.ok && d.encontrado){
        cb({ sku:d.sku, descripcion:d.descripcion||'' });
      } else {
        // Buscar en sku_catalog por sku
        bodFetch('GET','/api/cdg/v2/sku-catalog/buscar?q='+encodeURIComponent(q2))
          .then(function(d2){
            var r = d2 && d2.results && d2.results[0];
            cb(r ? { sku:r.sku, descripcion:r.descripcion||'' } : { sku:q2, descripcion:'' });
          }).catch(function(){ cb({ sku:q2, descripcion:'' }); });
      }
    }).catch(function(){ cb({ sku:q2, descripcion:'' }); });
}
...
function bodMostrarMenuPrincipal(){
  if(!window.BOD_ENABLED_CLIENT){ var c=document.getElementById('mc-bodega'); if(c)c.remove(); return; }
  if(!window.currentUser){ var c=document.getElementById('mc-bodega'); if(c)c.remove(); return; }
  var role = window.currentUser.role;
  if(role!=='bodega'&&role!=='supervisor'&&role!=='contador'){ var c=document.getElementById('mc-bodega'); if(c)c.remove(); return; }
  if(window.ACCESS_SCOPE==='bodega') return; // bodega va directo a pg-bod-menu
  var mcBod = document.getElementById('mc-bodega');
  if(!mcBod){
    var menuCards = document.querySelector('#pg-menu .menu-grid');
    if(!menuCards) return;
    var card = document.createElement('div');
    card.className='menu-card'; card.id='mc-bodega';
...
function bodAbrirMenu(){
  ...
  if(barraBtn) barraBtn.style.display = isBodAuditor() ? '' : 'none';
  if(!window._bodMenuWired){
    window._bodMenuWired = true;
    if(captBtn) captBtn.addEventListener('click',function(){ navTo('pg-bod-captura','📦 Capturar tarima'); bodRenderCaptura(); });
    if(audBtn)  audBtn.addEventListener('click',function(){ navTo('pg-bod-auditoria','🔍 Auditar sesión'); _bodAudSesion=null; bodRenderAuditoria(); });
    if(barraBtn) barraBtn.addEventListener('click',function(){ navTo('pg-bod-barra-sku','📋 Catálogo Barra-SKU'); bodRenderBarraSku(); });
  }
}

window.bodMostrarMenuPrincipal = bodMostrarMenuPrincipal;
...
window.bodCleanup = function(){
  _bodCapturaSesion        = null;
  _bodAudSesion            = null;
  _bodAudPtActivo          = null;
  _bodTeorico              = {};
  _bodTeoricoGlobalCargado = false;
  window._bodMenuWired = false;
  var el; 
  el=document.getElementById('bod-captura-contenido'); if(el) el.innerHTML='';
  el=document.getElementById('bod-aud-contenido');     if(el) el.innerHTML='';
  el=document.getElementById('bod-barra-contenido');   if(el) el.innerHTML='';
};

// ══════════════════════════════════════════════════════════════════════════
//  CATÁLOGO BARRA-SKU
// ══════════════════════════════════════════════════════════════════════════
function bodRenderBarraSku(){
  var el=document.getElementById('bod-barra-contenido'); if(!el) return;
  el.innerHTML='<div style="font-size:.82rem;font-weight:700;color:#333;text-transform:uppercase;letter-spacing:.3px;margin-bottom:.8rem">Catálogo Barra-SKU</div>'
    +'<p style="font-size:.82rem;color:#555;margin-bottom:.8rem">Cargá un Excel con columnas <strong>sku</strong> y <strong>descripcion</strong> (y opcionalmente <strong>barra</strong>) para actualizar el catálogo.</p>'
    +'<input type="file" id="bod-barra-file" accept=".xlsx,.xls" style="font-size:.82rem;margin-bottom:.6rem"><br>'
    +'<button class="btn btn-sm btn-dark" id="bod-barra-upload-btn">⬆ Subir catálogo</button>'
    +'<div id="bod-barra-msg" style="font-size:.78rem;margin-top:.5rem"></div>';
  document.getElementById('bod-barra-upload-btn').addEventListener('click',function(){
    var fi=document.getElementById('bod-barra-file');
    var msg=document.getElementById('bod-barra-msg');
    if(!fi||!fi.files[0]){ msg.innerHTML='<span style="color:#c0392b">Seleccioná un archivo.</span>'; return; }
    msg.innerHTML='<span style="color:#888">Subiendo…</span>';
    var fd=new FormData(); fd.append('file',fi.files[0]); fd.append('usuario',bodCurrentUser());
    fetch('/api/bod/barra-sku/upload',{method:'POST',body:fd}).then(function(r){return r.json();})
      .then(function(d){ msg.innerHTML=d.ok?'<span style="color:#1e7e34">✅ '+d.procesadas+' registros cargados</span>':'<span style="color:#c0392b">Error: '+bodEscA(d.error)+'</span>'; })
      .catch(function(){ msg.innerHTML='<span style="color:#c0392b">Error de red.</span>'; });
  });
}
...
function bodRenderCaptura(){
  var el=document.getElementById('bod-captura-contenido'); if(!el) return;
  el.innerHTML='<div style="color:#888;font-size:.82rem">Cargando licencias…</div>';
  bodFetch('GET','/api/bod/sesiones?limit=50')
    .then(function(d){ bodRenderPanelLicencias(el, d.ok?(d.sesiones||[]):[]); })
    .catch(function(){ el.innerHTML='<div style="color:#c0392b">Error de conexión.</div>'; });
}

function bodRenderPanelLicencias(el, sesiones){
  var usr = bodCurrentUser();
  var h = '<div style="font-size:.82rem;font-weight:700;color:#333;text-transform:uppercase;letter-spacing:.3px;margin-bottom:.8rem">Panel de Licencias Bolsón</div>';
  // Crear nueva licencia
  h += '<div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;margin-bottom:1rem;background:#f8f9fa;border-radius:8px;padding:.65rem .9rem">'
    +'<span style="font-size:.78rem;font-weight:600;color:#555">Nueva licencia:</span>'
    +'<input type="text" id="bod-nueva-lic" placeholder="U25-####" style="width:110px;font-size:.82rem;padding:4px 8px;border:1px solid #ddd;border-radius:6px" autocomplete="off" autocapitalize="characters">'
    +'<input type="date" id="bod-nueva-fecha" value="'+bodHoyGT()+'" style="font-size:.82rem;padding:4px 8px;border:1px solid #ddd;border-radius:6px">'
    +'<button class="btn btn-sm btn-dark" id="bod-crear-lic-btn">＋ Crear Licencia</button>'
    +'<span id="bod-crear-msg" style="font-size:.76rem"></span>'
    +'</div>';
  // Tabla de licencias
  h += '<div style="overflow-x:auto"><table style="width:100%;font-size:.78rem;border-collapse:collapse">'
    +'<thead><tr style="background:#f0f0f0;font-weight:700">'
    +'<th style="padding:6px 8px;text-align:left">Licencia Bolsón</th>'
    +'<th style="padding:6px 8px;text-align:left">Fecha</th>'
    +'<th style="padding:6px 8px;text-align:center">SKUs</th>'
    +'<th style="padding:6px 8px;text-align:center">Acciones</th>'
    +'</tr></thead><tbody id="bod-lic-tbody">';
  if(!sesiones.length){
    h+='<tr><td colspan="4" style="padding:12px 8px;color:#aaa;text-align:center">Sin licencias creadas</td></tr>';
  } else {
    sesiones.forEach(function(s){
      h+='<tr style="border-bottom:1px solid #f0f0f0">'
        +'<td style="padding:6px 8px;font-weight:600">'+bodEscA(s.licencia_id||s.id)+'</td>'
        +'<td style="padding:6px 8px;color:#555">'+bodEscA(bodFmtFecha(s.fecha_trabajo))+'</td>'
        +'<td style="padding:6px 8px;text-align:center">'+bodEscA(s.total_skus||0)+'</td>'
        +'<td style="padding:6px 8px;text-align:center;white-space:nowrap">'
        +'<button class="btn btn-xs b-empezar" data-id="'+bodEscA(s.id)+'" data-lic="'+bodEscA(s.licencia_id||'')+'" data-fecha="'+bodEscA(s.fecha_trabajo||'')+'" style="background:#1565c0;color:#fff;margin-right:4px">▶ Empezar</button>'
        +'<button class="btn btn-xs b-borrar-lic" data-id="'+bodEscA(s.id)+'" data-lic="'+bodEscA(s.licencia_id||'')+'" style="background:#c0392b;color:#fff">✕ Borrar</button>'
        +'</td></tr>';
    });
  }
  h += '</tbody></table></div>';
  ...
  // Crear licencia
  document.getElementById('bod-crear-lic-btn').addEventListener('click',function(){
    var licInput = document.getElementById('bod-nueva-lic');
    var fechaInput = document.getElementById('bod-nueva-fecha');
    var msg = document.getElementById('bod-crear-msg');
    var lic = (licInput.value||'').trim().toUpperCase();
    var fecha = (fechaInput.value||'').trim();
    if(!lic){ msg.innerHTML='<span style="color:#c0392b">Ingresá la licencia (ej: U25-1234).</span>'; return; }
    if(!fecha){ msg.innerHTML='<span style="color:#c0392b">Ingresá la fecha.</span>'; return; }
    msg.innerHTML='<span style="color:#888">Creando…</span>';
    bodFetch('POST','/api/bod/sesion',{ licencia_id:lic, fecha_trabajo:fecha, tipo:'recoleccion', creado_por:usr })
      .then(function(d){
        if(d.ok){
          msg.innerHTML='<span style="color:#1e7e34">✅ Licencia lista</span>';
          setTimeout(function(){ bodRenderCaptura(); },600);
        } else {
          msg.innerHTML='<span style="color:#c0392b">'+bodEscA(d.error||'Error')+'</span>';
        }
      }).catch(function(){ msg.innerHTML='<span style="color:#c0392b">Error de red.</span>'; });
  });

  // Empezar captura
  el.querySelectorAll('.b-empezar').forEach(function(btn){
    btn.addEventListener('click',function(){
      _bodCapturaSesion = { id:btn.dataset.id, licencia_id:btn.dataset.lic, fecha_trabajo:btn.dataset.fecha };
      bodRenderPanelCaptura(el);
    });
  });

  // Borrar licencia
  el.querySelectorAll('.b-borrar-lic').forEach(function(btn){
    btn.addEventListener('click',function(){
      if(!confirm('¿Borrar licencia '+btn.dataset.lic+'? Se eliminarán todas sus líneas.')) return;
      bodFetch('DELETE','/api/bod/sesion/'+encodeURIComponent(btn.dataset.id),{ usuario:usr })
        .then(function(d){
          if(d.ok){ bodRenderCaptura(); }
          else alert('No se pudo borrar: '+(d.error||'Error'));
        }).catch(function(){ alert('Error de red.'); });
    });
  });
}

function bodRenderPanelCaptura(el){
  var ses = _bodCapturaSesion;
  if(!ses){ bodRenderCaptura(); return; }
  var usr = bodCurrentUser();
  var prefix = bodPrefijo(usr);

  el.innerHTML = '<div style="font-size:.82rem;font-weight:700;color:#1565c0;margin-bottom:.4rem">📦 Captura — <span style="color:#333">'+bodEscA(ses.licencia_id||ses.id)+'</span>'
    +' <span style="color:#888;font-weight:400;font-size:.75rem">'+bodEscA(bodFmtFecha(ses.fecha_trabajo))+'</span>'
    +' <button id="bod-cap-volver" style="margin-left:8px;font-size:.72rem;background:none;border:1px solid #ccc;border-radius:4px;padding:1px 6px;cursor:pointer;color:#555">↩ Volver</button></div>'
    // Bloque de ingreso
    +'<div style="background:#f8f9fa;border-radius:8px;padding:.65rem .9rem;margin-bottom:.8rem">'
    +'<div style="display:flex;gap:8px;align-items:flex-end;flex-wrap:wrap">'
    +'<div><label style="font-size:.73rem;font-weight:600;display:block;margin-bottom:2px">Cód. Barra / SKU</label>'
    +'<input type="text" id="cap-barra" placeholder="Escanear o escribir" style="width:150px;font-size:.84rem;padding:5px 8px;border:1px solid #ddd;border-radius:6px" autocomplete="off" autocorrect="off"></div>'
    +'<div><label style="font-size:.73rem;font-weight:600;display:block;margin-bottom:2px">SKU</label>'
    +'<input type="text" id="cap-sku" placeholder="SKU" style="width:120px;font-size:.84rem;padding:5px 8px;border:1px solid #ddd;border-radius:6px" autocomplete="off" autocapitalize="characters"></div>'
    +'<div style="flex:1;min-width:140px"><label style="font-size:.73rem;font-weight:600;display:block;margin-bottom:2px">Descripción</label>'
    +'<input type="text" id="cap-desc" placeholder="Descripción" style="width:100%;font-size:.84rem;padding:5px 8px;border:1px solid #ddd;border-radius:6px" autocomplete="off"></div>'
    +'<div><label style="font-size:.73rem;font-weight:600;display:block;margin-bottom:2px">Tarima</label>'
    +'<input type="text" id="cap-tarima" placeholder="'+prefix+'01" style="width:70px;font-size:.84rem;padding:5px 8px;border:1px solid #ddd;border-radius:6px" autocomplete="off"></div>'
    +'<div><label style="font-size:.73rem;font-weight:600;display:block;margin-bottom:2px">Cantidad</label>'
    +'<input type="number" id="cap-cant" placeholder="0" min="1" style="width:80px;font-size:.84rem;padding:5px 8px;border:1px solid #ddd;border-radius:6px"></div>'
    +'<button class="btn btn-sm btn-dark" id="cap-agregar-btn" style="align-self:flex-end">+ Agregar</button>'
    +'</div>'
    +'<div id="cap-linea-msg" style="font-size:.76rem;margin-top:.35rem"></div>'
    +'</div>'
    // Tabla
    +'<div style="overflow-x:auto"><table style="width:100%;font-size:.76rem;border-collapse:collapse" id="cap-tabla">'
    +'<thead><tr style="background:#f0f0f0;font-weight:700">'
    +'<th style="padding:5px 7px;text-align:left">Tarima</th>'
    +'<th style="padding:5px 7px;text-align:left">SKU</th>'
    +'<th style="padding:5px 7px;text-align:left">Descripción</th>'
    +'<th style="padding:5px 7px;text-align:center">Cant.</th>'
    +'<th style="padding:5px 7px;text-align:left">Operador</th>'
    +'<th style="padding:5px 7px;text-align:center">Acción</th>'
    +'</tr></thead><tbody id="cap-tbody"><tr><td colspan="6" style="padding:10px;color:#aaa;text-align:center">Cargando…</td></tr></tbody>'
    +'</table></div>';

  document.getElementById('bod-cap-volver').addEventListener('click',function(){
    _bodCapturaSesion = null; bodRenderCaptura();
  });

  // Barra/SKU autocomplete
  var barraI = document.getElementById('cap-barra');
  var skuI   = document.getElementById('cap-sku');
  var descI  = document.getElementById('cap-desc');
  var _bt    = null;
  barraI.addEventListener('input',function(){
    clearTimeout(_bt);
    var v = barraI.value.trim();
    if(v.length >= 3){
      _bt = setTimeout(function(){
        bodLookupSku(v, ses.id, function(r){
          if(r){ skuI.value=r.sku||''; if(r.descripcion) descI.value=r.descripcion; }
        });
      }, 400);
    }
  });
  barraI.addEventListener('keydown',function(e){
    if(e.key==='Enter'){ document.getElementById('cap-cant').focus(); }
  });

  // Agregar línea
  document.getElementById('cap-agregar-btn').addEventListener('click',function(){
    var msg   = document.getElementById('cap-linea-msg');
    var barra = barraI.value.trim();
    var sku   = skuI.value.trim().toUpperCase() || barra.toUpperCase();
    var desc  = descI.value.trim();
    var tarima= (document.getElementById('cap-tarima').value||'').trim().toUpperCase();
    var cant  = Number(document.getElementById('cap-cant').value);
    if(!sku)   { msg.innerHTML='<span style="color:#c0392b">Ingresá el SKU.</span>'; return; }
    if(!tarima){ msg.innerHTML='<span style="color:#c0392b">Ingresá la tarima (ej: '+prefix+'01).</span>'; return; }
    if(!(cant>0)){ msg.innerHTML='<span style="color:#c0392b">La cantidad debe ser mayor a 0.</span>'; return; }
    msg.innerHTML='<span style="color:#888">Guardando…</span>';
    bodFetch('POST','/api/bod/sesion/'+encodeURIComponent(ses.id)+'/linea',{
      tarima:tarima, barra:barra||null, sku:sku, descripcion:desc, cantidad:cant, operador:usr
    }).then(function(d){
      if(d.ok){
        msg.innerHTML='<span style="color:#1e7e34">✅ Línea agregada</span>';
        barraI.value=''; skuI.value=''; descI.value='';
        document.getElementById('cap-tarima').value='';
        document.getElementById('cap-cant').value='';
        barraI.focus();
        bodCargarLineasCaptura(ses.id);
      } else {
        msg.innerHTML='<span style="color:#c0392b">'+bodEscA(d.error||'Error')+'</span>';
      }
    }).catch(function(){ msg.innerHTML='<span style="color:#c0392b">Error de red.</span>'; });
...
function bodCargarLineasCaptura(sesId){
  var tbody = document.getElementById('cap-tbody'); if(!tbody) return;
  var usr = bodCurrentUser();
  bodFetch('GET','/api/bod/sesion/'+encodeURIComponent(sesId)+'/lineas')
    .then(function(d){
      var lineas = d.ok?(d.lineas||[]):[];
      if(!lineas.length){ tbody.innerHTML='<tr><td colspan="6" style="padding:10px;color:#aaa;text-align:center">Sin líneas. Agregá la primera.</td></tr>'; return; }
      // Ordenar por tarima luego ts
      lineas.sort(function(a,b){ return (a.tarima||'').localeCompare(b.tarima||'')||(a.ts_captura||'').localeCompare(b.ts_captura||''); });
      tbody.innerHTML = lineas.map(function(l){
        var esMia = (l.operador||'')=== usr;
        return '<tr style="border-bottom:1px solid #f5f5f5">'
          +'<td style="padding:5px 7px;font-weight:600">'+bodEscA(l.tarima||'')+'</td>'
          +'<td style="padding:5px 7px">'+bodEscA(l.sku||'')+'</td>'
          +'<td style="padding:5px 7px;color:#555">'+bodEscA(l.descripcion||'')+'</td>'
          +'<td style="padding:5px 7px;text-align:center">'+bodEscA(l.cantidad||0)+'</td>'
          +'<td style="padding:5px 7px;color:#888">'+bodEscA(l.operador||'')+'</td>'
          +'<td style="padding:5px 7px;text-align:center">'
          +(esMia
            ? '<button class="btn btn-xs b-borrar-lin" data-id="'+bodEscA(l.id)+'" style="background:#c0392b;color:#fff">✕</button>'
            : '<span style="color:#ccc;font-size:.72rem">—</span>')
          +'</td></tr>';
      }).join('');
      tbody.querySelectorAll('.b-borrar-lin').forEach(function(btn){
        btn.addEventListener('click',function(){
          if(!confirm('¿Borrar esta línea?')) return;
          bodFetch('DELETE','/api/bod/linea/'+encodeURIComponent(btn.dataset.id),{ usuario:usr })
            .then(function(d){ if(d.ok) bodCargarLineasCaptura(sesId); else alert(d.error||'Error'); })
            .catch(function(){ alert('Error de red.'); });
        });
      });
    }).catch(function(){ if(tbody) tbody.innerHTML='<tr><td colspan="6" style="padding:10px;color:#c0392b">Error al cargar líneas.</td></tr>'; });
}
...
// ══════════════════════════════════════════════════════════════════════════
//  AUDITAR SESIÓN — Bodega CDG v3 (global por fecha, sin sesion_id)
//  Fuente: Bloqque_Bodega_CDG.docx + index de referencia (jun-2026)
//  Endpoints usados:
//    GET/POST/DELETE /api/bod/teorico-952-global
//    GET/POST/PATCH/DELETE /api/bod/pts
//    GET/POST/DELETE /api/bod/pts/:ptId/papel/:sku
//    POST/GET /api/bod/pts/:ptId/licencia-hija
//    GET /api/bod/sesiones (para Tab 1 y Tab 2 — captura sigue con sesion_id)
//    GET /api/bod/sesion/:id/lineas
// ══════════════════════════════════════════════════════════════════════════

// ── Estado global del módulo auditoría ───────────────────────────────────
var _audDB = {
  teorico:  [],   // array { sku, nombre, cantidad_952 }
  pts:      [],   // array de PTs del día
  ptActivo: null, // id del PT activo en Tab 5
  filtroTarima: null  // { sesId, tarima } — set al clicar Auditar en Tab 1
};
...
function _audBuscarTeo(sku){
  var v=(sku||'').trim().toUpperCase();
  for(var i=0;i<_audDB.teorico.length;i++) if((_audDB.teorico[i].sku||'').toUpperCase()===v) return _audDB.teorico[i];
  return null;
}

function _audGetPT(){
  if(!_audDB.ptActivo) return null;
  for(var i=0;i<_audDB.pts.length;i++) if(_audDB.pts[i].id===_audDB.ptActivo) return _audDB.pts[i];
  return null;
}

// ── Entrada principal ─────────────────────────────────────────────────────
function bodRenderAuditoria(){
  var el=document.getElementById('bod-aud-contenido'); if(!el) return;
  // Cargar teórico del día en memoria si no está cargado
  if(!_audDB.teorico.length){
    bodFetch('GET','/api/bod/teorico-952-global?fecha='+_audFecha()).then(function(d){
      _audDB.teorico=d.ok?(d.rows||[]):[];
    }).catch(function(){});
  }
  bodRenderAudTabs(el);
}
window.bodRenderAuditoria = bodRenderAuditoria;

// ── Tabs ──────────────────────────────────────────────────────────────────
function bodRenderAudTabs(el){
  var tab=_bodAudTab;
  var tabs=[
    {id:'1-tarimas',lbl:'1. Tarimas'},
    {id:'2-detalle',lbl:'2. Detalle'},
    {id:'3-teorico',lbl:'3. Carga Teórico'},
    {id:'4-control',lbl:'4. Control de Manifiestos'},
    {id:'5-papel',  lbl:'5. Papel de Trabajo'}
  ];
  var tabsH='<div style="display:flex;gap:3px;flex-wrap:wrap;margin-bottom:.6rem">';
  tabs.forEach(function(t){
    var on=t.id===tab;
    tabsH+='<button class="bod-tab-pill" data-tab="'+t.id+'" style="font-size:.73rem;padding:4px 10px;border-radius:4px 4px 0 0;border:1px solid #ddd;border-bottom:none;cursor:pointer;background:'+(on?'#1565c0':'#f5f5f5')+';color:'+(on?'#fff':'#555')+'">'+t.lbl+'</button>';
  });
  tabsH+='</div>';
  el.innerHTML=tabsH+'<div id="bod-tab-body"></div>';
  el.querySelectorAll('.bod-tab-pill').forEach(function(btn){
    btn.addEventListener('click',function(){
      _bodAudTab=btn.dataset.tab;
      bodRenderAudTabs(el);
    });
  });
  var tb=document.getElementById('bod-tab-body'); if(!tb) return;
  if(tab==='1-tarimas')      bodAudTab1(tb);
  else if(tab==='2-detalle') bodAudTab2(tb);
  else if(tab==='3-teorico') bodAudTab3(tb);
  else if(tab==='4-control') bodAudTab4(tb);
  else if(tab==='5-papel')   bodAudTab5(tb);
}

// ── Tab 1: Tarimas — todas las sesiones, filtros, botón Auditar ───────────
function bodAudTab1(tb){
  tb.innerHTML='<div style="color:#888;font-size:.82rem">Cargando tarimas…</div>';
  bodFetch('GET','/api/bod/sesiones?limit=100').then(function(dSes){
    var sesiones=dSes.ok?(dSes.sesiones||[]):[];
    if(!sesiones.length){
      tb.innerHTML='<div style="color:#aaa;font-size:.82rem;padding:.5rem">Sin sesiones. Creá una en Captura de Tarima.</div>';
      return;
    }
    Promise.all(sesiones.map(function(s){
      return bodFetch('GET','/api/bod/sesion/'+encodeURIComponent(s.id)+'/lineas')
        .then(function(d){return{ses:s,lineas:d.ok?(d.lineas||[]):[]}; })
        .catch(function(){return{ses:s,lineas:[]};});
    })).then(function(results){
      // Construir filas: una por (sesión, tarima)
      var rows=[];
      results.forEach(function(r){
        ...
        r.lineas.forEach(function(l){
          var t=l.tarima||'?';
          if(!byTar[t]) byTar[t]={tarima:t,lineas:[],ops:[]};
          byTar[t].lineas.push(l);
          if(byTar[t].ops.indexOf(l.operador||'')<0) byTar[t].ops.push(l.operador||'');
        });
        Object.values(byTar).forEach(function(t){
          var skus=[...new Set(t.lineas.map(function(l){return l.sku;}))];
          var auditados=t.lineas.filter(function(l){return l.auditado;}).length;
          rows.push({
            sesId:s.id, licencia:s.licencia_id||s.id, fecha:s.fecha_trabajo||'',
            tarima:t.tarima, skusUnicos:skus.length, lineas:t.lineas.length,
            auditados:auditados, operador:t.ops.filter(Boolean).join(', '),
            creado_por:s.creado_por||''
          });
        });
      });
      rows.sort(function(a,b){return a.licencia.localeCompare(b.licencia)||a.tarima.localeCompare(b.tarima);});

      // Filtros
      var fH='<div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:.6rem;align-items:center">'
        +'<input type="text" id="t1-f-lic" placeholder="Licencia bolsón" style="font-size:.76rem;padding:3px 7px;border:1px solid #ddd;border-radius:5px;width:130px">'
        +'<input type="date" id="t1-f-fecha" style="font-size:.76rem;padding:3px 7px;border:1px solid #ddd;border-radius:5px">'
        +'<input type="text" id="t1-f-tar" placeholder="Correlativo" style="font-size:.76rem;padding:3px 7px;border:1px solid #ddd;border-radius:5px;width:100px">'
        +'<button class="btn btn-xs" id="t1-f-clear" style="font-size:.72rem;background:#f5f5f5">✕ Limpiar</button>'
        +'<button class="btn btn-xs" id="t1-f-ref" style="font-size:.72rem;background:#e8f5e9;color:#1e7e34">↺</button>'
        +'</div>';

      if(!rows.length){tb.innerHTML=fH+'<div style="color:#aaa;font-size:.82rem">Sin tarimas capturadas.</div>';return;}

      var h=fH+'<div style="overflow-x:auto"><table style="width:100%;font-size:.74rem;border-collapse:collapse">'
        +'<thead><tr style="background:#f0f0f0;font-weight:700">'
        +'<th style="padding:5px 7px">Licencia Bolsón</th>'
        +'<th style="padding:5px 7px">Fecha</th>'
        +'<th style="padding:5px 7px;text-align:center">SKUs únicos</th>'
        +'<th style="padding:5px 7px;text-align:center">Auditados</th>'
        +'<th style="padding:5px 7px">Correlativo Tarima</th>'
        +'<th style="padding:5px 7px;text-align:center">Líneas</th>'
        +'<th style="padding:5px 7px">Operador</th>'
        +'<th style="padding:5px 7px;text-align:center">Acción</th>'
        +'</tr></thead><tbody id="t1-tbody">';
      rows.forEach(function(r){
        h+='<tr class="t1-row" data-lic="'+bodEscA(r.licencia)+'" data-fecha="'+bodEscA(r.fecha)+'" data-tar="'+bodEscA(r.tarima)+'">'
          +'<td style="padding:5px 7px;font-weight:600;color:#1565c0">'+bodEscA(r.licencia)+'</td>'
          +'<td style="padding:5px 7px;color:#666">'+bodEscA(bodFmtFecha(r.fecha))+'</td>'
          +'<td style="padding:5px 7px;text-align:center">'+r.skusUnicos+'</td>'
          +'<td style="padding:5px 7px;text-align:center">'+r.auditados+'</td>'
          +'<td style="padding:5px 7px;font-weight:600">'+bodEscA(r.tarima)+'</td>'
          +'<td style="padding:5px 7px;text-align:center">'+r.lineas+'</td>'
          +'<td style="padding:5px 7px;color:#888;font-size:.71rem">'+bodEscA(r.operador)+'</td>'
          +'<td style="padding:5px 7px;text-align:center">'
          +'<button class="btn btn-xs b-aud-tar" data-sesid="'+bodEscA(r.sesId)+'" data-tarima="'+bodEscA(r.tarima)+'" style="background:#1565c0;color:#fff">🔍 Auditar</button>'
          +'</td></tr>';
      });
      h+='</tbody></table></div>';
      tb.innerHTML=h;
      ...
      function f1(){
        var fL=(document.getElementById('t1-f-lic')||{value:''}).value.trim().toUpperCase();
        var fF=(document.getElementById('t1-f-fecha')||{value:''}).value.trim();
        var fT=(document.getElementById('t1-f-tar')||{value:''}).value.trim().toUpperCase();
        tb.querySelectorAll('.t1-row').forEach(function(row){
          row.style.display=(!fL||row.dataset.lic.toUpperCase().includes(fL))
            &&(!fF||row.dataset.fecha===fF)&&(!fT||row.dataset.tar.toUpperCase().includes(fT))?'':'none';
        });
      }
      ['t1-f-lic','t1-f-fecha','t1-f-tar'].forEach(function(id){
        var e=document.getElementById(id); if(e) e.addEventListener('input',f1);
      });
      var clr=document.getElementById('t1-f-clear');
      if(clr) clr.addEventListener('click',function(){
        ['t1-f-lic','t1-f-fecha','t1-f-tar'].forEach(function(id){var e=document.getElementById(id);if(e)e.value='';});
        f1();
      });
      var ref=document.getElementById('t1-f-ref');
      if(ref) ref.addEventListener('click',function(){ bodAudTab1(tb); });

      tb.querySelectorAll('.b-aud-tar').forEach(function(btn){
        btn.addEventListener('click',function(){
          _audDB.filtroTarima={sesId:btn.dataset.sesid, tarima:btn.dataset.tarima};
          _bodAudTab='2-detalle';
          var elMain=document.getElementById('bod-aud-contenido');
          if(elMain) bodRenderAudTabs(elMain);
        });
      });

    }).catch(function(){tb.innerHTML='<div style="color:#c0392b">Error al cargar tarimas.</div>';});
  }).catch(function(){tb.innerHTML='<div style="color:#c0392b">Error al cargar sesiones.</div>';});
}

// ── Tab 2: Detalle — todas las sesiones, filtros ─────────────────────────
function bodAudTab2(tb){
  tb.innerHTML='<div style="color:#888;font-size:.82rem">Cargando detalle…</div>';
  var filtroTarIni = _audDB.filtroTarima ? _audDB.filtroTarima.tarima : '';
  var usr = bodCurrentUser();
  _audDB.filtroTarima = null; // consumir filtro
  ...
  bodFetch('GET','/api/bod/sesiones?limit=100').then(function(dSes){
    ...
    Promise.all(sesiones.map(function(s){
      ...
      results.forEach(function(r){
        r.lineas.forEach(function(l){
          allLineas.push(Object.assign({},l,{_lic:r.ses.licencia_id||r.ses.id,_fecha:r.ses.fecha_trabajo||'',_sesId:r.ses.id}));
        });
      });
      allLineas.sort(function(a,b){return (a._lic||'').localeCompare(b._lic||'')||(a.tarima||'').localeCompare(b.tarima||'');});

      var fH='<div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:.6rem;align-items:center">'
        +'<input type="text" id="t2-f-lic" placeholder="Licencia bolsón" style="font-size:.76rem;padding:3px 7px;border:1px solid #ddd;border-radius:5px;width:130px">'
        +'<input type="date" id="t2-f-fecha" style="font-size:.76rem;padding:3px 7px;border:1px solid #ddd;border-radius:5px">'
        +'<input type="text" id="t2-f-tar" placeholder="Tarima" style="font-size:.76rem;padding:3px 7px;border:1px solid #ddd;border-radius:5px;width:80px" value="'+bodEscA(filtroTarIni)+'">'
        +'<input type="text" id="t2-f-sku" placeholder="SKU" style="font-size:.76rem;padding:3px 7px;border:1px solid #ddd;border-radius:5px;width:100px">'
        +'<button class="btn btn-xs" id="t2-f-clear" style="font-size:.72rem;background:#f5f5f5">✕ Limpiar</button>'
        +'<button class="btn btn-xs" id="t2-f-ref" style="font-size:.72rem;background:#e8f5e9;color:#1e7e34">↺</button>'
        +'</div>';

      if(!allLineas.length){tb.innerHTML=fH+'<div style="color:#aaa;font-size:.82rem">Sin líneas.</div>';return;}

      var h=fH+'<div style="overflow-x:auto"><table style="width:100%;font-size:.73rem;border-collapse:collapse">'
        +'<thead><tr style="background:#f0f0f0;font-weight:700">'
        +'<th style="padding:5px 6px">Licencia</th><th style="padding:5px 6px">Fecha</th>'
        +'<th style="padding:5px 6px">Tarima</th><th style="padding:5px 6px">SKU</th>'
        +'<th style="padding:5px 6px">Descripción</th>'
        +'<th style="padding:5px 6px;text-align:center">Cant.</th>'
        +'<th style="padding:5px 6px;text-align:center">Físico</th>'
        +'<th style="padding:5px 6px;text-align:center">Dif.</th>'
        +'<th style="padding:5px 6px;text-align:center">Estado</th>'
        +'<th style="padding:5px 6px">Operador</th>'
        +'<th style="padding:5px 6px">Auditado por</th>'
        +'<th style="padding:5px 6px;text-align:center">Acción</th>'
        +'</tr></thead><tbody id="t2-tbody">';

      allLineas.forEach(function(l){
        var fis=l.cantidad_audit!==null&&l.cantidad_audit!==undefined?Number(l.cantidad_audit):null;
        var cant=Number(l.cantidad)||0;
        var dif=fis!==null?(cant-fis):null;
        var est=dif===null?'—':(dif===0?'Cuadrado':dif<0?'Sobrante':'Faltante');
        var estC=dif===null?'#888':(dif===0?'#1e7e34':dif<0?'#1565c0':'#c0392b');
        var aud=!!l.auditado;
        h+='<tr class="t2-row" data-id="'+bodEscA(l.id)+'" data-sesid="'+bodEscA(l._sesId)+'" data-lic="'+bodEscA(l._lic)+'" data-fecha="'+bodEscA(l._fecha)+'" data-tar="'+bodEscA(l.tarima||'')+'" data-sku="'+bodEscA(l.sku||'')+'">'
          +'<td style="padding:5px 6px;color:#1565c0;font-weight:600">'+bodEscA(l._lic)+'</td>'
          +'<td style="padding:5px 6px;color:#666;font-size:.7rem">'+bodEscA(bodFmtFecha(l._fecha))+'</td>'
          +'<td style="padding:5px 6px;font-weight:600">'+bodEscA(l.tarima||'')+'</td>'
          +'<td style="padding:5px 6px">'+bodEscA(l.sku||'')+'</td>'
          +'<td style="padding:5px 6px;color:#555;max-width:130px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">'+bodEscA(l.descripcion||'')+'</td>'
          +'<td style="padding:5px 6px;text-align:center">'+cant+'</td>'
          +'<td style="padding:5px 6px;text-align:center"><input type="number" class="t2-fis" value="'+(fis!==null?fis:'')+'" placeholder="—" min="0" style="width:52px;font-size:.73rem;padding:2px 4px;border:1px solid #ddd;border-radius:4px;text-align:center"'+(aud?' disabled':'')+'></td>'
          +'<td style="padding:5px 6px;text-align:center;font-weight:600;color:'+estC+'">'+(dif!==null?dif:'—')+'</td>'
          +'<td style="padding:5px 6px;text-align:center;font-size:.7rem;color:'+estC+'">'+est+'</td>'
          +'<td style="padding:5px 6px;color:#888;font-size:.7rem">'+bodEscA(l.operador||'')+'</td>'
          +'<td style="padding:5px 6px;color:#555;font-size:.7rem">'+bodEscA(l.auditado_por||'')+'</td>'
          +'<td style="padding:5px 6px;text-align:center">'
          +(aud
            ?'<button class="btn btn-xs b-anular-aud" data-id="'+bodEscA(l.id)+'" data-sesid="'+bodEscA(l._sesId)+'" style="background:#c0392b;color:#fff;font-size:.68rem">Anular</button>'
            :'<button class="btn btn-xs b-auditar-lin" data-id="'+bodEscA(l.id)+'" data-sesid="'+bodEscA(l._sesId)+'" style="background:#1e7e34;color:#fff;font-size:.68rem">Auditar</button>'
          )+'</td></tr>';
      });
      h+='</tbody></table></div>';
      tb.innerHTML=h;
      ...
      function f2(){
        var fL=(document.getElementById('t2-f-lic')||{value:''}).value.trim().toUpperCase();
        var fF=(document.getElementById('t2-f-fecha')||{value:''}).value.trim();
        var fT=(document.getElementById('t2-f-tar')||{value:''}).value.trim().toUpperCase();
        var fS=(document.getElementById('t2-f-sku')||{value:''}).value.trim().toUpperCase();
        tb.querySelectorAll('.t2-row').forEach(function(row){
          row.style.display=(!fL||row.dataset.lic.toUpperCase().includes(fL))
            &&(!fF||row.dataset.fecha===fF)&&(!fT||row.dataset.tar.toUpperCase().includes(fT))
            &&(!fS||row.dataset.sku.toUpperCase().includes(fS))?'':'none';
        });
      }
      ['t2-f-lic','t2-f-fecha','t2-f-tar','t2-f-sku'].forEach(function(id){
        var e=document.getElementById(id); if(e) e.addEventListener('input',f2);
      });
      var clr=document.getElementById('t2-f-clear');
      if(clr) clr.addEventListener('click',function(){
        ['t2-f-lic','t2-f-fecha','t2-f-tar','t2-f-sku'].forEach(function(id){var e=document.getElementById(id);if(e)e.value='';});
        f2();
      });
      var ref=document.getElementById('t2-f-ref');
      if(ref) ref.addEventListener('click',function(){bodAudTab2(tb);});
      if(filtroTarIni) f2();

      // Auditar
      tb.querySelectorAll('.b-auditar-lin').forEach(function(btn){
        btn.addEventListener('click',function(){
          var row=btn.closest('tr');
          var inp=row.querySelector('.t2-fis');
          if(!inp||inp.value===''){alert('Ingresá el físico antes de auditar.');inp&&inp.focus();return;}
          var fisico=Number(inp.value);
          btn.disabled=true;btn.textContent='…';
          var sesId=btn.dataset.sesid;
          bodFetch('PATCH','/api/bod/linea/'+encodeURIComponent(btn.dataset.id)+'/fisico-audit',
            {fisico_auditoria:fisico,teorico_952:0,usuario:usr})
            .then(function(d1){
              if(!d1.ok){alert(d1.error||'Error');btn.disabled=false;btn.textContent='Auditar';return;}
              bodFetch('POST','/api/bod/linea/'+encodeURIComponent(btn.dataset.id)+'/auditar',{usuario:usr})
                .then(function(d2){if(d2.ok)bodAudTab2(tb);else{alert(d2.error||'Error');btn.disabled=false;btn.textContent='Auditar';}})
                .catch(function(){btn.disabled=false;btn.textContent='Auditar';});
            }).catch(function(){btn.disabled=false;btn.textContent='Auditar';alert('Error de red.');});
        });
      });
      tb.querySelectorAll('.b-anular-aud').forEach(function(btn){
        btn.addEventListener('click',function(){
          if(!confirm('¿Anular la auditoría?')) return;
          bodFetch('POST','/api/bod/linea/'+encodeURIComponent(btn.dataset.id)+'/unauditar',{usuario:usr,auditor:true})
            .then(function(d){if(d.ok)bodAudTab2(tb);else alert(d.error||'Error');})
            .catch(function(){alert('Error de red.');});
        });
      });
    }).catch(function(){tb.innerHTML='<div style="color:#c0392b">Error al cargar detalle.</div>';});
  }).catch(function(){tb.innerHTML='<div style="color:#c0392b">Error al cargar sesiones.</div>';});
}

// ── Tab 3: Carga Teórico — global por fecha ───────────────────────────────
function bodAudTab3(tb){
  var usr=bodCurrentUser();
  var fecha=_audFecha();
  bodFetch('GET','/api/bod/teorico-952-global?fecha='+fecha).then(function(d){
    var rows=d.ok?(d.rows||[]):[];
    _audDB.teorico=rows; // actualizar en memoria
    var h='<div style="font-size:.82rem;font-weight:700;color:#333;margin-bottom:.4rem">Carga Teórico 952</div>'
      +'<p style="font-size:.78rem;color:#555;margin-bottom:.5rem">Subí el archivo <strong>952_006_001_Teórico.xlsx</strong>. Columnas: SKU, Nombre, Cant. 952, Unidad, Exist., Disp.</p>'
      +'<div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;margin-bottom:.6rem">'
      +'<input type="file" id="tab3-file" accept=".xlsx,.xls" style="font-size:.78rem">'
      +'<button class="btn btn-sm btn-dark" id="tab3-up-btn">⬆ Subir Teórico</button>'
      +(rows.length?'<button class="btn btn-xs" id="tab3-clr-btn" style="background:#c0392b;color:#fff">🗑 Limpiar</button>':'')
      +'<span id="tab3-msg" style="font-size:.76rem"></span></div>'
      +'<div style="font-size:.74rem;color:#888;margin-bottom:.3rem">Fecha: '+bodEscA(fecha)+' — '+rows.length+' SKUs cargados</div>';
    if(rows.length){
      h+='<div style="overflow-x:auto;max-height:280px"><table style="width:100%;font-size:.72rem;border-collapse:collapse">'
        +'<thead><tr style="background:#f0f0f0;font-weight:700"><th style="padding:4px 6px">SKU</th><th style="padding:4px 6px">Nombre</th><th style="padding:4px 6px;text-align:center">Cant. 952</th><th style="padding:4px 6px">Unidad</th></tr></thead><tbody>';
      rows.slice(0,300).forEach(function(r){
        h+='<tr style="border-bottom:1px solid #f5f5f5"><td style="padding:3px 6px">'+bodEscA(r.sku)+'</td><td style="padding:3px 6px;color:#555">'+bodEscA(r.nombre||'')+'</td><td style="padding:3px 6px;text-align:center">'+bodEscA(r.cantidad_952||0)+'</td><td style="padding:3px 6px;color:#888">'+bodEscA(r.unidades||'')+'</td></tr>';
      });
      if(rows.length>300) h+='<tr><td colspan="4" style="padding:4px;color:#aaa;text-align:center">…y '+(rows.length-300)+' más</td></tr>';
      h+='</tbody></table></div>';
    } else {
      h+='<div style="color:#aaa;font-size:.82rem">Sin teórico cargado para hoy.</div>';
    }
    tb.innerHTML=h;

    document.getElementById('tab3-up-btn').addEventListener('click',function(){
      var fi=document.getElementById('tab3-file');
      var msg=document.getElementById('tab3-msg');
      if(!fi||!fi.files[0]){msg.innerHTML='<span style="color:#c0392b">Seleccioná el archivo.</span>';return;}
      msg.innerHTML='<span style="color:#888">Subiendo…</span>';
      var fd=new FormData(); fd.append('file',fi.files[0]); fd.append('fecha',fecha); fd.append('creado_por',usr);
      fetch('/api/bod/teorico-952-global/upload',{method:'POST',body:fd})
        .then(function(r){return r.json();})
        .then(function(d){
          if(d.ok){
            msg.innerHTML='<span style="color:#1e7e34">✅ '+d.procesadas+' SKUs cargados</span>';
            setTimeout(function(){bodAudTab3(tb);},600);
          } else msg.innerHTML='<span style="color:#c0392b">Error: '+bodEscA(d.error)+'</span>';
        }).catch(function(){msg.innerHTML='<span style="color:#c0392b">Error de red.</span>';});
    });
    var clrBtn=document.getElementById('tab3-clr-btn');
    if(clrBtn) clrBtn.addEventListener('click',function(){
      if(!confirm('¿Limpiar el teórico del día?')) return;
      bodFetch('DELETE','/api/bod/teorico-952-global?fecha='+fecha).then(function(d){
        if(d.ok){_audDB.teorico=[];bodAudTab3(tb);}else alert(d.error||'Error');
      }).catch(function(){alert('Error de red.');});
    });
  }).catch(function(){tb.innerHTML='<div style="color:#c0392b">Error al cargar teórico.</div>';});
}

// ── Tab 4: Control de Manifiestos — PTs globales por fecha ────────────────
function bodAudTab4(tb){
  var usr=bodCurrentUser();
  var fecha=_audFecha();
  tb.innerHTML='<div style="color:#888;font-size:.82rem">Cargando…</div>';
  bodFetch('GET','/api/bod/pts?fecha='+fecha).then(function(d){
    _audDB.pts=d.ok?(d.pts||[]):[];

    var h='<div style="margin-bottom:.7rem;display:flex;gap:.5rem;align-items:center">'
      +'<button class="btn btn-sm btn-dark" id="t4-gen-pt">＋ Generar PT</button>'
      +'<span id="t4-gen-msg" style="font-size:.76rem"></span></div>';

    if(!_audDB.pts.length){
      h+='<div style="color:#aaa;font-size:.82rem">Sin PTs. Presioná "Generar PT".</div>';
    } else {
      // TABLA HORIZONTAL — igual que la referencia y el Word
      h+='<div style="overflow-x:auto"><table style="width:100%;font-size:.75rem;border-collapse:collapse;white-space:nowrap">'
        +'<thead><tr style="background:#f0f0f0;font-weight:700">'
        +'<th style="padding:5px 7px;text-align:left">Correlativo</th>'
        +'<th style="padding:5px 7px;text-align:left">Estado</th>'
        +'<th style="padding:5px 7px">Furgón</th>'
        +'<th style="padding:5px 7px">Placa</th>'
        +'<th style="padding:5px 7px">Marchamo</th>'
        +'<th style="padding:5px 7px">Lic. Hija</th>'
        +'<th style="padding:5px 7px">TR999</th>'
        +'<th style="padding:5px 7px">Observaciones</th>'
        +'<th style="padding:5px 7px">Carga</th>'
        +'<th style="padding:5px 7px">Creador</th>'
        +'<th style="padding:5px 7px">Auditor</th>'
        +'<th style="padding:5px 7px">Colaboradores</th>'
        +'<th style="padding:5px 7px;text-align:left">Acciones</th>'
        +'</tr></thead><tbody id="t4-tbody">';

      _audDB.pts.forEach(function(m){
        var bloq=m.estado==='finalizado'||m.estado==='anulado';
        // Estado badge
        var estC=m.estado==='finalizado'?'#1e7e34':m.estado==='anulado'?'#c0392b':'#1565c0';
        var estL=m.estado==='finalizado'?'Finalizado':m.estado==='anulado'?'Anulado':'En Proceso';
        // Campos: readonly por defecto, editables cuando el usuario hace clic en Editar
        // Usamos data-editing="false" para controlar estado
        var rdonly = bloq ? ' disabled' : ' readonly';
        h+='<tr class="t4-pt-row" data-mid="'+bodEscA(m.id)+'" data-editing="false" style="border-bottom:1px solid #eee;vertical-align:middle">'
          +'<td style="padding:5px 7px;font-weight:700">'+bodEscA(m.correlativo||m.id)+'</td>'
          +'<td style="padding:5px 7px"><span style="font-size:.71rem;color:'+estC+';border:1px solid '+estC+';border-radius:10px;padding:1px 8px">'+estL+'</span></td>'
          // Campos inline editables
          +t4TD('furgon',m.furgon||'',rdonly,'70px')
          +t4TD('placa',m.placa||'',rdonly,'80px')
          +t4TD('marchamo',m.marchamo||'',rdonly,'80px')
          +t4TD('licencia_hija',m.licencia_hija||'',rdonly,'90px')
          +t4TD('destino_tr999',m.destino_tr999||'',rdonly,'110px')
          +t4TD('observacion',m.observacion||'',rdonly,'100px')
          +t4TD('carga_no',m.carga_no||'',rdonly,'70px')
          +'<td style="padding:5px 7px;color:#888;font-size:.72rem">'+bodEscA(m.creado_por||'')+'</td>'
          +t4TD('auditor_lider',m.auditor_lider||'',rdonly,'90px')
          +'<td style="padding:5px 7px;color:#888;font-size:.72rem">'+bodEscA((m.colaboradores||[]).join(', ')||'—')+'</td>'
          // Acciones
          +'<td style="padding:5px 7px;white-space:nowrap">'
          +'<div style="display:flex;gap:4px;flex-wrap:wrap">'
          +(!bloq?'<button class="btn btn-xs b-t4-edit" data-mid="'+bodEscA(m.id)+'" style="font-size:.69rem">Editar</button>':'')
          +(!bloq?'<button class="btn btn-xs b-t4-continuar" data-mid="'+bodEscA(m.id)+'" style="background:#1565c0;color:#fff;font-size:.69rem">'+(m.auditor_lider?'Continuar':'Empezar')+'</button>':'')
          +(m.estado==='finalizado'?'<button class="btn btn-xs b-t4-reabrir" data-mid="'+bodEscA(m.id)+'" style="font-size:.69rem">Reabrir</button>':'')
          +'<button class="btn btn-xs b-t4-lich" data-mid="'+bodEscA(m.id)+'" style="font-size:.69rem">Subir Lic. Hija WMS</button>'
          +(m.estado==='finalizado'?'<button class="btn btn-xs b-t4-manif" data-mid="'+bodEscA(m.id)+'" style="background:#1e7e34;color:#fff;font-size:.69rem">Generar Manifiesto</button>':'')
          +(!bloq?'<button class="btn btn-xs b-t4-borrar" data-mid="'+bodEscA(m.id)+'" style="background:#c0392b;color:#fff;font-size:.69rem">Borrar</button>':'')
          +'</div>'
          // Upload licencia hija (oculto)
          +'<div class="t4-lich-wrap" data-mid="'+bodEscA(m.id)+'" style="display:none;margin-top:4px;padding:4px;background:#f8f9fa;border-radius:5px">'
          +'<input type="file" class="t4-lich-file" accept=".xlsx,.xls" style="font-size:.73rem">'
          +'<button class="btn btn-xs t4-lich-up-btn" data-mid="'+bodEscA(m.id)+'" style="margin-left:4px">⬆ Subir</button>'
          +'<span class="t4-lich-msg" style="font-size:.72rem;margin-left:4px"></span>'
          +'</div>'
          +'<div class="t4-pt-msg" style="font-size:.73rem;margin-top:2px;color:#c0392b"></div>'
          +'</td>'
          +'</tr>';
      });
      h+='</tbody></table></div>';
    ...
    // Generar PT
    document.getElementById('t4-gen-pt').addEventListener('click',function(){
      var msg=document.getElementById('t4-gen-msg');
      msg.innerHTML='<span style="color:#888">Generando…</span>';
      bodFetch('POST','/api/bod/pts',{fecha:fecha,creado_por:usr})
        .then(function(d){
          if(d.ok){
            msg.innerHTML='<span style="color:#1e7e34">✅ '+bodEscA(d.pt&&d.pt.correlativo||'PT creado')+'</span>';
            setTimeout(function(){bodAudTab4(tb);},500);
          } else msg.innerHTML='<span style="color:#c0392b">'+bodEscA(d.error||'Error')+'</span>';
        }).catch(function(){msg.innerHTML='<span style="color:#c0392b">Error de red.</span>';});
    });

    // Editar / Guardar cambios (toggle)
    tb.querySelectorAll('.b-t4-edit').forEach(function(btn){
      btn.addEventListener('click',function(){
        var row=btn.closest('.t4-pt-row');
        var mId=btn.dataset.mid;
        var isEditing=row.dataset.editing==='true';
        if(!isEditing){
          // Habilitar campos
          row.querySelectorAll('.t4-campo-inp').forEach(function(inp){inp.removeAttribute('readonly');});
          btn.textContent='Guardar cambios';
          btn.style.background='#1565c0'; btn.style.color='#fff';
          row.dataset.editing='true';
        } else {
          // Guardar
          var datos={usuario:usr};
          row.querySelectorAll('.t4-campo-inp').forEach(function(inp){datos[inp.dataset.campo]=inp.value.trim();});
          var msg=row.querySelector('.t4-pt-msg');
          if(msg) msg.textContent='Guardando…';
          bodFetch('PATCH','/api/bod/pts/'+encodeURIComponent(mId),datos)
            .then(function(d){
              if(d.ok){
                // Bloquear campos de nuevo
                row.querySelectorAll('.t4-campo-inp').forEach(function(inp){inp.setAttribute('readonly','');});
                btn.textContent='Editar';
                btn.style.background=''; btn.style.color='';
                row.dataset.editing='false';
                if(msg) msg.textContent='';
                // Actualizar _audDB.pts local
                var ptLocal=_audDB.pts.find(function(p){return p.id===mId;});
                if(ptLocal) Object.assign(ptLocal,datos);
              } else {
                if(msg) msg.textContent=d.error||'Error al guardar';
              }
    ...
    // Empezar/Continuar → Tab 5
    tb.querySelectorAll('.b-t4-continuar').forEach(function(btn){
      btn.addEventListener('click',function(){
        _audDB.ptActivo=btn.dataset.mid;
        _bodAudTab='5-papel';
        var elMain=document.getElementById('bod-aud-contenido');
        if(elMain) bodRenderAudTabs(elMain);
      });
    ...
    // Reabrir PT
    tb.querySelectorAll('.b-t4-reabrir').forEach(function(btn){
      btn.addEventListener('click',function(){
        bodFetch('PATCH','/api/bod/pts/'+encodeURIComponent(btn.dataset.mid),{usuario:usr,estado:'en_proceso'})
          .then(function(d){if(d.ok)bodAudTab4(tb);else alert(d.error||'Error');})
          .catch(function(){alert('Error de red.');});
      });
    });

    // Subir Lic. Hija WMS
    tb.querySelectorAll('.b-t4-lich').forEach(function(btn){
      btn.addEventListener('click',function(){
        var mId=btn.dataset.mid;
        var wrap=tb.querySelector('.t4-lich-wrap[data-mid="'+mId+'"]');
        if(wrap) wrap.style.display=wrap.style.display==='none'?'':'none';
      });
    });
    tb.querySelectorAll('.t4-lich-up-btn').forEach(function(btn){
      btn.addEventListener('click',function(){
        var mId=btn.dataset.mid;
        var wrap=tb.querySelector('.t4-lich-wrap[data-mid="'+mId+'"]');
        var fi=wrap?wrap.querySelector('.t4-lich-file'):null;
        var msg=wrap?wrap.querySelector('.t4-lich-msg'):null;
        if(!fi||!fi.files[0]){if(msg)msg.textContent='Seleccioná el archivo.';return;}
        if(msg)msg.textContent='Subiendo…';
        var fd=new FormData(); fd.append('file',fi.files[0]); fd.append('creado_por',usr);
        fetch('/api/bod/pts/'+encodeURIComponent(mId)+'/licencia-hija/upload',{method:'POST',body:fd})
          .then(function(r){return r.json();})
          .then(function(d){if(msg)msg.textContent=d.ok?('✅ '+d.procesadas+' filas cargadas'):('Error: '+(d.error||'?'));})
          .catch(function(){if(msg)msg.textContent='Error de red.';});
      });
    });

    // Generar Manifiesto
    tb.querySelectorAll('.b-t4-manif').forEach(function(btn){
      btn.addEventListener('click',function(){
        var mId=btn.dataset.mid;
        var pt=_audDB.pts.find(function(p){return p.id===mId;})||{};
        var row=btn.closest('.t4-pt-row');
        var msg=row?row.querySelector('.t4-pt-msg'):null;
        if(msg) msg.textContent='Preparando…';
        Promise.all([
          bodFetch('GET','/api/bod/pts/'+encodeURIComponent(mId)+'/papel'),
          bodFetch('GET','/api/bod/pts/'+encodeURIComponent(mId)+'/licencia-hija').catch(function(){return{ok:true,rows:[]};})
        ]).then(function(rs){
          var lineas=rs[0].ok?(rs[0].rows||[]):[];
          var lich=rs[1].ok?(rs[1].rows||[]):[];
          if(!lineas.length){if(msg)msg.textContent='El PT no tiene líneas.';return;}
          var lichMap={};
          lich.forEach(function(r){lichMap[r.sku]=(Number(r.cantidad)||0);});
          if(msg) msg.textContent='';
          audMostrarSelectorExport(lineas,lichMap,pt);
        }).catch(function(){if(msg)msg.textContent='Error de red.';});
      });
    });

    // Borrar PT
    tb.querySelectorAll('.b-t4-borrar').forEach(function(btn){
      btn.addEventListener('click',function(){
        if(!confirm('¿Borrar este PT?')) return;
        bodFetch('DELETE','/api/bod/pts/'+encodeURIComponent(btn.dataset.mid))
          .then(function(d){if(d.ok)bodAudTab4(tb);else alert(d.error||'Error');})
          .catch(function(){alert('Error de red.');});
      });
    });

  }).catch(function(){tb.innerHTML='<div style="color:#c0392b">Error al cargar PTs.</div>';});
}

// Celda de campo editable en la tabla de PTs
function t4TD(campo,valor,rdonly,width){
  return '<td style="padding:4px 6px">'
    +'<input type="text" class="t4-campo-inp" data-campo="'+campo+'" value="'+bodEscA(valor)+'"'
    +' style="width:'+width+';font-size:.73rem;padding:2px 5px;border:1px solid #ddd;border-radius:4px"'
    +rdonly+' autocomplete="off"></td>';
}

// ── Selector de formato de exportación ───────────────────────────────────
function audMostrarSelectorExport(lineas,lichMap,pt){
  ...
  ov.innerHTML='<div style="background:#fff;border-radius:12px;padding:1.4rem;width:310px;max-width:95vw;box-shadow:0 8px 32px rgba(0,0,0,.2)">'
    +'<h3 style="font-size:.9rem;font-weight:700;margin:0 0 .3rem">Generar Manifiesto</h3>'
    +'<p style="font-size:.77rem;color:#666;margin:0 0 .9rem">'+bodEscA(pt.correlativo||'')+'</p>'
    +'<p style="font-size:.78rem;font-weight:600;margin:0 0 .35rem">Versión:</p>'
    +'<div style="display:flex;flex-direction:column;gap:.3rem;margin-bottom:.7rem">'
    +'<button id="exp-manif-xls" class="btn btn-sm" style="background:#1e7e34;color:#fff">📊 Excel — Manifiesto (Físico / WMS)</button>'
    +'<button id="exp-muestra-xls" class="btn btn-sm" style="background:#1565c0;color:#fff">📊 Excel — Muestra (WMS / Físico)</button>'
    +'<button id="exp-manif-csv" class="btn btn-sm" style="background:#555;color:#fff">📋 CSV — Manifiesto</button>'
    +'<button id="exp-muestra-csv" class="btn btn-sm" style="background:#777;color:#fff">📋 CSV — Muestra</button>'
    +'</div>'
    +'<button id="exp-cancel" class="btn btn-xs" style="width:100%;background:#f5f5f5;color:#555">Cancelar</button>'
    +'</div>';
  document.body.appendChild(ov);
  ov.addEventListener('click',function(e){if(e.target===ov)ov.remove();});
  document.getElementById('exp-cancel').addEventListener('click',function(){ov.remove();});
  document.getElementById('exp-manif-xls' ).addEventListener('click',function(){ov.remove();audGenerarExcel(lineas,lichMap,pt,false);});
  document.getElementById('exp-muestra-xls').addEventListener('click',function(){ov.remove();audGenerarExcel(lineas,lichMap,pt,true);});
  document.getElementById('exp-manif-csv' ).addEventListener('click',function(){ov.remove();audGenerarCSV(lineas,lichMap,pt,false);});
  document.getElementById('exp-muestra-csv').addEventListener('click',function(){ov.remove();audGenerarCSV(lineas,lichMap,pt,true);});
}

// ── Generador Excel — formato del ejemplo del Word ────────────────────────
// Manifiesto normal : CANTIDAD = Físico,  columna "WMS"    = dato WMS/licencia hija
// Muestra           : CANTIDAD = WMS,     columna "VALIDADO"= Físico
function audGenerarExcel(lineas,lichMap,pt,esMuestra){
  if(typeof XLSX==='undefined'){alert('Librería XLSX no disponible en el navegador.');return;}

  // % auditado por SKU (cantidad auditada / total del SKU)
  var totalPorSku={}, audPorSku={};
  lineas.forEach(function(s){
    var fis=Number(s.fisico_auditoria||0);
    totalPorSku[s.sku]=(totalPorSku[s.sku]||0)+fis;
    if(s.estado_papel==='Auditado') audPorSku[s.sku]=(audPorSku[s.sku]||0)+fis;
  });

  var fecha=new Date().toLocaleDateString('es',{day:'2-digit',month:'2-digit',year:'numeric'});
  var colWMS=esMuestra?'VALIDADO':'WMS';

  // Filas del Excel — mismo formato que el ejemplo adjunto
  var rows=[
    ['','','CARGA ENVÍO A BODEGA','','','','',''],
    ['','','','','','','',''],
    ['','','','','','Marchamo:',pt.marchamo||'',''],
    ['','Herramientas Poderosas','','','','Licencia:',pt.licencia_hija||'',''],
    ['','','DIRECCIÓN:','27 Calle Bodega C 41-55 Zona 5 Calzada la Paz','','UBICACIÓN:',pt.destino_tr999||'',''],
    ['','DIRECCIÓN DESTINO:','BODEGA NODUS (Hamilón)','','Fecha:',fecha,'',''],
    ['','','','','','Carga No.',pt.carga_no||'',''],
    ['','','','','','','',''],
    ['','CODIGO','ESTATUS','DESCRIPCION','CANTIDAD',colWMS,'DIFERENCIA','']
  ];

  lineas.forEach(function(s){
    var fis=Number(s.fisico_auditoria||0);
    var wms=lichMap[s.sku]||0;
    var tot=totalPorSku[s.sku]||1;
    var pct=Math.round(((audPorSku[s.sku]||0)/tot)*100)+'%';
    var cant=esMuestra?wms:fis;
    var val=esMuestra?fis:wms;
    var dif=wms-fis;
    rows.push(['',s.sku,s.estado_papel||'No auditado',s.nombre||'',cant,val,dif,'']);
  });

  rows.push(['','','','','','','','']);
  rows.push(['','','','','','','','']);
  rows.push(['','AUTORIZACIÓN DE ENVÍO','','NOMBRE Y FIRMA QUIEN RECIBE','','','','']);
  rows.push(['','ASTRID DUARTE','','','','','','']);
  rows.push(['','CINTYA RIVERA','TH 243','','','','','']);
  rows.push(['','HUVALDO PEREZ','TRANSPORTE','','','','','']);
  rows.push(['','','','','','','','']);
  rows.push(['','ENCARGADO DE PEDIDO','','AUDITOR / VALIDADOR','','','','']);
  rows.push(['','HUVALDO PEREZ','','pt.auditor_lider' in pt ? pt.auditor_lider : '','','','','']);

  var ws=XLSX.utils.aoa_to_sheet(rows);
  var wb=XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb,ws,'Manifiesto');
  var fn=((pt.correlativo||'manifiesto').replace(/[^a-zA-Z0-9_-]/g,'_'))+(esMuestra?'-muestra':'-manifiesto')+'.xlsx';
  XLSX.writeFile(wb,fn);
}

// ── Generador CSV ─────────────────────────────────────────────────────────
function audGenerarCSV(lineas,lichMap,pt,esMuestra){
  var totalPorSku={}, audPorSku={};
  lineas.forEach(function(s){
    var fis=Number(s.fisico_auditoria||0);
    totalPorSku[s.sku]=(totalPorSku[s.sku]||0)+fis;
  ...
  var colWMS=esMuestra?'VALIDADO':'WMS';
  var lines=['\uFEFF'+'CODIGO,ESTATUS,DESCRIPCION,CANTIDAD,'+colWMS+',DIFERENCIA'];
  lineas.forEach(function(s){
    var fis=Number(s.fisico_auditoria||0);
    var wms=lichMap[s.sku]||0;
    var cant=esMuestra?wms:fis;
    var val=esMuestra?fis:wms;
    var dif=wms-fis;
    lines.push(['"'+s.sku+'"','"'+(s.estado_papel||'No auditado')+'"','"'+(s.nombre||'')+'"',cant,val,dif].join(','));
  });
  var a=document.createElement('a');
  a.href=URL.createObjectURL(new Blob([lines.join('\n')],{type:'text/csv;charset=utf-8;'}));
  a.download=((pt.correlativo||'manifiesto').replace(/[^a-zA-Z0-9_-]/g,'_'))+(esMuestra?'-muestra':'-manifiesto')+'.csv';
  a.click();
}


function bodAudTab5(tb){
  if(!_audDB.ptActivo){
    tb.innerHTML='<div style="color:#888;font-size:.82rem;padding:.5rem">▶ Usá <strong>Empezar</strong> en Control de Manifiestos para trabajar el Papel de Trabajo.</div>';
    return;
  }
  tb.innerHTML='<div style="color:#888;font-size:.82rem">Cargando…</div>';
  var usr=bodCurrentUser();
  var mId=_audDB.ptActivo;
  var pt=_audDB.pts.find(function(p){return p.id===mId;})||{};
  var bloq=pt.estado==='finalizado'||pt.estado==='anulado';

  bodFetch('GET','/api/bod/pts/'+encodeURIComponent(mId)+'/papel').then(function(d){
    var lineas=d.ok?(d.rows||[]):[];

    // Banner PT
    var h='<div style="background:#f0f4ff;border-radius:8px;padding:.5rem .85rem;margin-bottom:.6rem;font-size:.75rem">'
      +'<strong>'+bodEscA(pt.correlativo||mId)+'</strong>'
      +(pt.furgon?' · Furgón: '+bodEscA(pt.furgon):'')
      +(pt.placa?' · Placa: '+bodEscA(pt.placa):'')
      +(pt.marchamo?' · Marchamo: '+bodEscA(pt.marchamo):'')
      +(pt.licencia_hija?' · Lic. Hija: '+bodEscA(pt.licencia_hija):'')
      +(pt.destino_tr999?' · TR999: '+bodEscA(pt.destino_tr999):'')
      +' <button id="pp-ir-ctrl" style="margin-left:8px;font-size:.7rem;background:none;border:1px solid #ccc;border-radius:3px;padding:1px 5px;cursor:pointer">↩ Control</button>'
      +'</div>';

    // Bloque agregar
    if(!bloq){
      h+='<div style="background:#f8f9fa;border-radius:8px;padding:.6rem .85rem;margin-bottom:.6rem">'
        +'<div style="font-size:.76rem;font-weight:700;color:#333;margin-bottom:.4rem">Agregar SKU</div>'
        +'<div style="display:flex;gap:7px;align-items:flex-end;flex-wrap:wrap">'
        +'<div><label style="font-size:.7rem;font-weight:600;display:block;margin-bottom:1px">Cód. Barra</label>'
        +'<input type="text" id="pp-barra" placeholder="Escanear" style="width:110px;font-size:.81rem;padding:4px 6px;border:1px solid #ddd;border-radius:5px" autocomplete="off"></div>'
        +'<div><label style="font-size:.7rem;font-weight:600;display:block;margin-bottom:1px">SKU</label>'
        +'<input type="text" id="pp-sku" placeholder="SKU" style="width:100px;font-size:.81rem;padding:4px 6px;border:1px solid #ddd;border-radius:5px" autocomplete="off"></div>'
        +'<div style="flex:1;min-width:120px"><label style="font-size:.7rem;font-weight:600;display:block;margin-bottom:1px">Descripción</label>'
        +'<input type="text" id="pp-desc" placeholder="Descripción" style="width:100%;font-size:.81rem;padding:4px 6px;border:1px solid #ddd;border-radius:5px"></div>'
        +'<div><label style="font-size:.7rem;font-weight:600;display:block;margin-bottom:1px">Físico</label>'
        +'<input type="number" id="pp-fisico" placeholder="0" min="0" style="width:68px;font-size:.81rem;padding:4px 6px;border:1px solid #ddd;border-radius:5px"></div>'
        +'<div><label style="font-size:.7rem;font-weight:600;display:block;margin-bottom:1px">Tarima</label>'
        +'<input type="text" id="pp-tarima" placeholder="A01" style="width:62px;font-size:.81rem;padding:4px 6px;border:1px solid #ddd;border-radius:5px" autocomplete="off"></div>'
        +'<div><label style="font-size:.7rem;font-weight:600;display:block;margin-bottom:1px">Estatus</label>'
        +'<select id="pp-est" style="font-size:.81rem;padding:4px 6px;border:1px solid #ddd;border-radius:5px">'
        +'<option value="Auditado">Auditado</option><option value="No auditado">No auditado</option></select></div>'
        +'<button class="btn btn-sm btn-dark" id="pp-add-btn" style="align-self:flex-end">+ Agregar</button>'
        +'</div><div id="pp-msg" style="font-size:.75rem;margin-top:.3rem"></div></div>';
    }

    // Tabla
    h+='<div style="overflow-x:auto"><table style="width:100%;font-size:.72rem;border-collapse:collapse">'
      +'<thead><tr style="background:#f0f0f0;font-weight:700">'
      +'<th style="padding:4px 5px">SKU</th><th style="padding:4px 5px">Descripción</th>'
      +'<th style="padding:4px 5px;text-align:center">Físico</th><th style="padding:4px 5px">Tarima</th>'
      +'<th style="padding:4px 5px">Estatus</th><th style="padding:4px 5px">Origen</th>'
      +'<th style="padding:4px 5px;text-align:center">952</th>'
      +'<th style="padding:4px 5px;text-align:center">Diferencia</th>'
      +'<th style="padding:4px 5px">Seguimiento</th>'
      +'<th style="padding:4px 5px;text-align:center" title="Validación">✓Val.</th>'
      +'<th style="padding:4px 5px;text-align:center">Dif.Post</th>'
      +'<th style="padding:4px 5px;text-align:center">Acción</th>'
      +'</tr></thead><tbody id="pp-tbody">';

    if(!lineas.length){
      h+='<tr><td colspan="12" style="padding:10px;color:#aaa;text-align:center">Sin líneas. Agregá el primer SKU.</td></tr>';
    } else {
      lineas.forEach(function(l){
        var teo=_audBuscarTeo(l.sku); var t952=teo?teo.cantidad_952:(l.cantidad_952||0);
        var fis=Number(l.fisico_auditoria)||0;
        var dif=t952-fis;
        var validado=!!l.validado;
        var difPost=validado?0:dif;
        var seg=dif===0?'Cuadrado':(dif<0?'Solicitar traslado a 952':'Se queda en bolsón');
        var segC=dif===0?'#1e7e34':(dif<0?'#c0392b':'#1565c0');
        var esMio=(l.creado_por||'')=== usr;
        h+='<tr class="pp-row" data-sku="'+bodEscA(l.sku)+'">'
          +'<td style="padding:4px 5px;font-weight:600">'+bodEscA(l.sku)+'</td>'
          +'<td style="padding:4px 5px;color:#555;max-width:120px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">'+bodEscA(l.nombre||'')+'</td>'
          +'<td style="padding:4px 5px;text-align:center">'+fis+'</td>'
          +'<td style="padding:4px 5px">'+bodEscA(l.tarimas||'')+'</td>'
          +'<td style="padding:4px 5px;font-size:.69rem">'+bodEscA(l.estado_papel||'')+'</td>'
          +'<td style="padding:4px 5px;font-size:.69rem;color:#888">'+bodEscA(l.origen||'creado')+'</td>'
          +'<td style="padding:4px 5px;text-align:center;color:#555">'+t952+'</td>'
          +'<td style="padding:4px 5px;text-align:center;font-weight:600;color:'+(dif===0?'#1e7e34':(dif<0?'#c0392b':'#1565c0'))+'">'+dif+'</td>'
          +'<td style="padding:4px 5px;font-size:.69rem;color:'+segC+'">'+seg+'</td>'
          +'<td style="padding:4px 5px;text-align:center">'
          +(bloq?(validado?'<span style="color:#1e7e34">✅</span>':'<span style="color:#ccc">—</span>')
            :'<input type="checkbox" class="pp-val-chk" data-sku="'+bodEscA(l.sku)+'" '+(validado?'checked':'')+'>')
          +'</td>'
          +'<td style="padding:4px 5px;text-align:center;font-weight:600;color:'+(difPost===0?'#1e7e34':'#c0392b')+'">'+difPost+'</td>'
          +'<td style="padding:4px 5px;text-align:center">'
          +((!bloq&&esMio)?'<button class="btn btn-xs pp-del-btn" data-sku="'+bodEscA(l.sku)+'" style="background:#c0392b;color:#fff">✕</button>':'<span style="color:#ccc">—</span>')
          +'</td></tr>';
      });
    }
    h+='</tbody></table></div>';
    ...
    if(!bloq){
      h+='<div style="margin-top:.7rem;display:flex;gap:.45rem;flex-wrap:wrap">'
        +'<button class="btn btn-sm" id="pp-fin-manif" style="background:#1565c0;color:#fff">✅ Finalizar como Manifiesto</button>'
        +'<button class="btn btn-sm" id="pp-fin-muestra" style="background:#7b241c;color:#fff">📋 Finalizar como Muestra</button>'
        +'</div><div id="pp-fin-msg" style="font-size:.75rem;margin-top:.3rem"></div>';
    }
    tb.innerHTML=h;

    // ↩ Control
    var irCtrl=document.getElementById('pp-ir-ctrl');
    if(irCtrl) irCtrl.addEventListener('click',function(){
      _bodAudTab='4-control';
      var elMain=document.getElementById('bod-aud-contenido');
      if(elMain) bodRenderAudTabs(elMain);
    });
    ...
    if(!bloq){
      // Autocomplete
      var ppB=document.getElementById('pp-barra');
      var ppS=document.getElementById('pp-sku');
      var ppD=document.getElementById('pp-desc');
      var _ppT=null;
      if(ppB) ppB.addEventListener('input',function(){
        clearTimeout(_ppT);
        var v=ppB.value.trim();
        if(v.length>=3){ _ppT=setTimeout(function(){
          bodLookupSku(v,null,function(r){
            if(r){ppS.value=r.sku||'';if(r.descripcion)ppD.value=r.descripcion;}
            if(r&&r.sku){var t=_audBuscarTeo(r.sku);if(t&&!ppD.value)ppD.value=t.nombre||'';}
          });
        },400);}
      });
      if(ppS) ppS.addEventListener('blur',function(){
        var s=ppS.value.trim().toUpperCase();
        if(s){var t=_audBuscarTeo(s);if(t&&!ppD.value)ppD.value=t.nombre||'';}
      });

      // Agregar SKU
      document.getElementById('pp-add-btn').addEventListener('click',function(){
        var msg=document.getElementById('pp-msg');
        var barra=ppB?ppB.value.trim():'';
        var sku=(ppS?ppS.value.trim().toUpperCase():'')||barra.toUpperCase();
        var desc=ppD?ppD.value.trim():'';
        var fis=Number((document.getElementById('pp-fisico')||{value:0}).value);
        var tarima=(document.getElementById('pp-tarima')||{value:''}).value.trim().toUpperCase();
        var est=(document.getElementById('pp-est')||{value:'No auditado'}).value;
        if(!sku){msg.innerHTML='<span style="color:#c0392b">Ingresá el SKU.</span>';return;}
        if(!(fis>=0)){msg.innerHTML='<span style="color:#c0392b">Ingresá el físico.</span>';return;}
        var teo=_audBuscarTeo(sku);
        var t952=teo?teo.cantidad_952:0;
        if(!desc&&teo) desc=teo.nombre||'';
        var dif=t952-fis;
        var seg=dif===0?'Cuadrado':(dif<0?'Solicitar traslado a 952':'Se queda en bolsón');
        msg.innerHTML='<span style="color:#888">Guardando…</span>';
        bodFetch('POST','/api/bod/pts/'+encodeURIComponent(mId)+'/papel',{
          usuario:usr,
          skus:[{sku:sku,nombre:desc,fisico_auditoria:fis,cantidad_952:t952,diferencia:dif,
                 seguimiento:seg,tarimas:tarima,estado_papel:est,origen:'creado',
                 validado:false,validado_por:'',creado_por:usr}]
        }).then(function(d){
          if(d.ok){
            msg.innerHTML='<span style="color:#1e7e34">✅ Agregado</span>';
            if(ppB)ppB.value='';if(ppS)ppS.value='';if(ppD)ppD.value='';
            document.getElementById('pp-fisico').value='';
            document.getElementById('pp-tarima').value='';
            bodAudTab5(tb);
          } else msg.innerHTML='<span style="color:#c0392b">'+bodEscA(d.error||'Error')+'</span>';
        }).catch(function(){msg.innerHTML='<span style="color:#c0392b">Error de red.</span>';});
      });
      ...
      // Validación checkbox
      tb.querySelectorAll('.pp-val-chk').forEach(function(chk){
        chk.addEventListener('change',function(){
          var sku=chk.dataset.sku;
          var linea=lineas.find(function(l){return l.sku===sku;});if(!linea)return;
          var teo=_audBuscarTeo(sku);var t952=teo?teo.cantidad_952:(linea.cantidad_952||0);
          var fis=Number(linea.fisico_auditoria)||0;
          var dif=t952-fis;
          bodFetch('POST','/api/bod/pts/'+encodeURIComponent(mId)+'/papel',{
            usuario:usr,
            skus:[{sku:sku,nombre:linea.nombre||'',fisico_auditoria:fis,cantidad_952:t952,diferencia:dif,
                   seguimiento:linea.seguimiento||'',tarimas:linea.tarimas||'',estado_papel:linea.estado_papel||'',
                   origen:linea.origen||'creado',validado:chk.checked,validado_por:chk.checked?usr:'',creado_por:linea.creado_por||usr}]
          }).then(function(d){if(d.ok)bodAudTab5(tb);}).catch(function(){});
        });
      });

      // Borrar SKU
      tb.querySelectorAll('.pp-del-btn').forEach(function(btn){
        btn.addEventListener('click',function(){
          if(!confirm('¿Borrar esta línea?')) return;
          bodFetch('DELETE','/api/bod/pts/'+encodeURIComponent(mId)+'/papel/'+encodeURIComponent(btn.dataset.sku))
            .then(function(d){if(d.ok)bodAudTab5(tb);else alert(d.error||'Error');})
            .catch(function(){alert('Error de red.');});
        });
      });

      // Finalizar
      function finalizarPP(esMuestra){
        var fMsg=document.getElementById('pp-fin-msg');
        if(!lineas.length){fMsg.innerHTML='<span style="color:#c0392b">Agregá líneas primero.</span>';return;}
        if(!confirm('¿Finalizar como '+(esMuestra?'Muestra':'Manifiesto')+'?')) return;
        fMsg.innerHTML='<span style="color:#888">Finalizando…</span>';
        bodFetch('PATCH','/api/bod/pts/'+encodeURIComponent(mId),{usuario:usr,estado:'finalizado'})
          .then(function(d){
            if(d.ok){
              fMsg.innerHTML='<span style="color:#1e7e34">✅ Finalizado. Generando…</span>';
              // Actualizar estado local
              var ptLocal=_audDB.pts.find(function(p){return p.id===mId;});
              if(ptLocal) ptLocal.estado='finalizado';
              // Generar manifiesto
              setTimeout(function(){
                var lichP=bodFetch('GET','/api/bod/pts/'+encodeURIComponent(mId)+'/licencia-hija').catch(function(){return{ok:true,rows:[]};});
                lichP.then(function(r){
                  var lich=r.ok?(r.rows||[]):[];
                  var lichMap={}; lich.forEach(function(lh){lichMap[lh.sku]=Number(lh.cantidad)||0;});
                  var ptLocal=_audDB.pts.find(function(p){return p.id===mId;})||{};
                  audMostrarSelectorExport({skus:lineas,lichMap:lichMap},ptLocal,esMuestra);
                });
                _bodAudTab='4-control';
                var elMain=document.getElementById('bod-aud-contenido');
                if(elMain) bodRenderAudTabs(elMain);
              },800);
            } else fMsg.innerHTML='<span style="color:#c0392b">'+bodEscA(d.error||'Error')+'</span>';
          }).catch(function(){fMsg.innerHTML='<span style="color:#c0392b">Error de red.</span>';});
      }
      document.getElementById('pp-fin-manif').addEventListener('click',function(){finalizarPP(false);});
      document.getElementById('pp-fin-muestra').addEventListener('click',function(){finalizarPP(true);});
    }
  }).catch(function(){tb.innerHTML='<div style="color:#c0392b">Error al cargar papel de trabajo.</div>';});
}
...
<script>
/* ══════════════════════════════════════════════════════════════════
   BOOMROOMS MODULE v2 — Conteos Hamilton (jul-2026)
   Módulo independiente. No toca Embarques, Traslados ni CDG.
   - 4 pestañas: Conteo, Reporte, Hallazgos, Asignaciones
   - Locks por tarima+índice igual que Embarques
   - Autoguardado y polling igual que Embarques
   - Teórico: hoja "Boomroom", E=SKU, F=Desc, G=Qty, H=Costo, I=UbicBR
   - TR en col I = pendiente, se ignora
   - SKUs extra: cantidad directa = físico, sin teórico
   - Re-subir teórico: agrega/actualiza tarimas sin tocar físicos ni fechas
══════════════════════════════════════════════════════════════════ */
(function(){
'use strict';

/* ── Estado ─────────────────────────────────────────────────────── */
var brTarima      = null;   // tarima seleccionada
var brClasifFiltro = 'todos';
var brSkuTimer    = null;   // debounce autocomplete

/* ── Helpers ─────────────────────────────────────────────────────── */
function brConts(){
  return conts.filter(function(c){ return teorico[c] && teorico[c].type==='BoomRoom'; });
}
function brEsc(s){ return escH ? escH(s) : String(s||'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;'); }
function brItems(t){ return (teorico[t] && teorico[t].items) || []; }
function brFisArr(t){ return Array.isArray(fisico[t]) ? fisico[t] : []; }
...
function brClasif(t){
  ...
  items.forEach(function(it,i){
    var sv = fis[i];
    if(sv && sv.fisico !== null && sv.fisico !== undefined) contados++;
  });
  return contados >= items.length ? 'Auditado' : 'Pendiente';
}

function brFechaTrabajo(t){
  var d = teorico[t] && teorico[t].fechaCarga;
  if(!d) return '';
...
function brSwitchTab(name){
  ['conteo','reporte','hallazgos','asign'].forEach(function(t){
    var el  = document.getElementById('br-tab-'+t);
    var btn = document.getElementById('br-tab-btn-'+t);
    if(el)  el.style.display  = (t===name) ? '' : 'none';
    if(btn) btn.classList.toggle('on', t===name);
  });
}
...
function brWireEvents(){
  // Tab buttons
  var btnC = document.getElementById('br-tab-btn-conteo');
  if(btnC && !btnC._brW){
    btnC._brW = true;
    btnC.addEventListener('click', function(){ brSwitchTab('conteo'); });

    document.getElementById('br-tab-btn-reporte').addEventListener('click', function(){
      brSwitchTab('reporte'); brRenderReporte();
    });
    document.getElementById('br-tab-btn-hallazgos').addEventListener('click', function(){
      brSwitchTab('hallazgos'); brRenderHallazgos();
    });
    var btnA = document.getElementById('br-tab-btn-asign');
    if(btnA) btnA.addEventListener('click', function(){
      if(!isSup()) return;
      brSwitchTab('asign'); brRenderAsign();
    });

    // Selector tarima
    document.getElementById('br-sel-tarima').addEventListener('change', function(){
      brTarima = this.value || null;
      // Cargar fecha existente de la tarima seleccionada
      var fechaInp = document.getElementById('br-fecha-trabajo');
      if(fechaInp && brTarima && teorico[brTarima] && teorico[brTarima].fechaCarga){
        fechaInp.value = teorico[brTarima].fechaCarga;
      } else if(fechaInp) {
        fechaInp.value = '';
      }
      brRenderConteo();
    ...
    // FIX (jul-2026, v2): el <datalist> nativo del navegador se percibía
    // "lento" (varios segundos) al escribir, porque el navegador filtra su
    // propio popup de sugerencias contra TODAS las opciones cargadas, y con
    // cientos/miles de tarimas ese filtrado nativo se vuelve pesado en
    // varios navegadores/dispositivos. Se reemplaza por un dropdown propio
    // en JS: se filtra en memoria (rápido) y se muestran máximo 25
    // resultados, evitando el cuello de botella del datalist.
    // El <select> original queda visible al lado como respaldo.
    var tarimaBuscar   = document.getElementById('br-tarima-buscar');
    var tarimaDropdown = document.getElementById('br-tarima-dropdown');
    if(tarimaBuscar && !tarimaBuscar._brW){
      tarimaBuscar._brW = true;

      var brHideDropdown = function(){
        if(tarimaDropdown){ tarimaDropdown.style.display='none'; tarimaDropdown.innerHTML=''; }
      };

      var brSelectTarima = function(val){
        var sel = document.getElementById('br-sel-tarima');
        if(sel) sel.value = val;
        brTarima = val || null;
        var fechaInp = document.getElementById('br-fecha-trabajo');
        if(fechaInp){
          fechaInp.value = (brTarima && teorico[brTarima] && teorico[brTarima].fechaCarga)
            ? teorico[brTarima].fechaCarga : '';
        }
        tarimaBuscar.value = val || '';
        brHideDropdown();
        brRenderConteo();
      };

      var brRenderTarimaDropdown = function(q){
        if(!tarimaDropdown) return;
        var brs = brConts();
        var ql = q.toLowerCase();
        var matches = q ? brs.filter(function(c){ return c.toLowerCase().indexOf(ql) >= 0; }) : brs;
        var total = matches.length;
        matches = matches.slice(0, 25); // limitar resultados -> mantiene el dropdown rápido
        if(!matches.length){ brHideDropdown(); return; }
        tarimaDropdown.innerHTML = matches.map(function(c){
          return '<div class="br-tarima-opt" data-v="'+brEsc(c)+'" style="padding:7px 12px;cursor:pointer;font-size:.85rem">'+brEsc(c)+'</div>';
        }).join('') + (total>25 ? '<div style="padding:5px 12px;font-size:.72rem;color:#999">'+(total-25)+' más — seguí escribiendo para acotar</div>' : '');
        tarimaDropdown.style.display = 'block';
        tarimaDropdown.querySelectorAll('.br-tarima-opt').forEach(function(opt){
          // mousedown (no click) para que dispare ANTES del blur del input
          opt.addEventListener('mousedown', function(e){
            e.preventDefault();
            brSelectTarima(this.dataset.v);
          });
          opt.addEventListener('mouseenter', function(){ this.style.background='#f2f2f2'; });
          opt.addEventListener('mouseleave', function(){ this.style.background=''; });
        });
      };

      tarimaBuscar.addEventListener('input', function(){
        var val = tarimaBuscar.value.trim();
        brRenderTarimaDropdown(val);
        var brs = brConts();
        if(val && brs.indexOf(val) >= 0){
          brSelectTarima(val); // coincidencia exacta -> seleccionar de inmediato
        } else if(!val && brTarima){
          brSelectTarima(''); // campo vaciado -> deseleccionar
        }
      });
      tarimaBuscar.addEventListener('focus', function(){ brRenderTarimaDropdown(tarimaBuscar.value.trim()); });
      tarimaBuscar.addEventListener('blur', function(){ setTimeout(brHideDropdown, 150); });
    }

    // Fecha de trabajo — guarda en teorico[brTarima].fechaCarga
    var brFechaInp = document.getElementById('br-fecha-trabajo');
    if(brFechaInp && !brFechaInp._brW) {
      brFechaInp._brW = true;
      brFechaInp.addEventListener('change', function(){
        if(!brTarima) return;
        if(!teorico[brTarima]) return;
        teorico[brTarima].fechaCarga = this.value;
        // Guardar en server via conteo/field con idx=-1 para que persista
        fetch('/api/conteo/field',{method:'POST',headers:{'Content-Type':'application/json'},
          body:JSON.stringify({cont:brTarima, idx:-2, fisico:null, daniado:null,
            cobertura:'En revision', usuario:currentUser?currentUser.name:'',
            fechaCarga:this.value})
        }).catch(function(){});
        brRenderReporte(); // actualizar reporte
      });
    }

    // SKU autocomplete
    document.getElementById('br-add-sku').addEventListener('input', function(){
      clearTimeout(brSkuTimer);
      var sku = this.value.trim();
      var descEl = document.getElementById('br-add-desc');
      var autoEl = document.getElementById('br-add-desc-auto');
      if(!sku){ descEl.value=''; descEl.style.background='#f5f5f3'; if(autoEl) autoEl.textContent=''; return; }
      brSkuTimer = setTimeout(function(){
        fetch('/api/cdg/v2/sku-catalog/buscar?sku='+encodeURIComponent(sku))
          .then(function(r){ return r.json(); })
          .then(function(d){
            if(d.encontrado){
              descEl.value = d.descripcion||'';
              descEl.style.background = '#f5f5f3';
              if(autoEl) autoEl.textContent = '✓ '+d.descripcion;
            } else {
              // buscar en teórico de cualquier tarima BR
              var found = '';
              brConts().forEach(function(c){
                if(found) return;
                var it = brItems(c).find(function(x){ return String(x.sku).trim()===sku; });
                if(it && it.desc) found = it.desc;
              });
              descEl.value = found;
              descEl.style.background = found ? '#f5f5f3' : '#fff';
              if(autoEl) autoEl.textContent = found ? '✓ del teórico' : '';
            }
          }).catch(function(){});
      }, 350);
    });

    // Botón Agregar SKU extra
    document.getElementById('br-btn-add-sku').addEventListener('click', function(){
      if(!brTarima || !currentUser) return;
      var sku  = (document.getElementById('br-add-sku').value||'').trim();
      var desc = (document.getElementById('br-add-desc').value||'').trim();
      var qty  = parseFloat(document.getElementById('br-add-qty').value);
      var msg  = document.getElementById('br-add-msg');
      if(!sku){ msg.innerHTML='<span style="color:#c0392b">Ingresá el SKU.</span>'; return; }
      if(isNaN(qty)||qty<0){ msg.innerHTML='<span style="color:#c0392b">Ingresá una cantidad válida.</span>'; return; }

      // Inicializar array si hace falta
      var items = brItems(brTarima);
      if(!Array.isArray(fisico[brTarima])){
        fisico[brTarima] = new Array(items.length).fill(null);
      }
      // Agregar al final del array (después de teóricos)
      fisico[brTarima].push({
        fisico:   qty,
        daniado:  null,
        cobertura:'Auditado',
        quien:    currentUser.name,
        ts:       new Date().toLocaleTimeString('es-GT',{hour:'2-digit',minute:'2-digit'}),
        lastAt:   Date.now(),
        extraSku: true,
        sku:      sku,
        desc:     desc||sku
      ...
      brSaveAll();
      msg.innerHTML='<span style="color:#1e7e34">✓ SKU agregado</span>';
      document.getElementById('br-add-sku').value = '';
      document.getElementById('br-add-desc').value = '';
      document.getElementById('br-add-qty').value = '';
      document.getElementById('br-add-desc-auto').textContent = '';
      brRenderConteo();
    });

    // Botón Crear hallazgo — usa el mismo modal que Embarques
    var btnHall = document.getElementById('br-btn-hallazgo');
    if(btnHall && !btnHall._brW) {
      btnHall._brW = true;
      btnHall.addEventListener('click', function(){
        if(!brTarima){ alert('Seleccioná una tarima primero.'); return; }
        // Setear el contenedor actual al de la tarima BoomRoom
        var hallName = document.getElementById('hall-cont-name');
        if(hallName) hallName.textContent = brTarima;
        ['hall-input','hall-condicion','hall-criterio','hall-causa','hall-efecto','hall-recomendacion'].forEach(function(id){
          var el=document.getElementById(id); if(el) el.value='';
        });
        var preview=document.getElementById('hall-foto-preview');
        var img=document.getElementById('hall-foto-img');
        var status=document.getElementById('hall-foto-status');
        var inp=document.getElementById('hall-foto-input');
        if(preview) preview.style.display='none';
        if(img) img.src='';
        if(status) status.textContent='';
        if(inp) inp.value='';
        window._hallFotoData=null;
        window._editingHallId=null;
        window._hallContOverride=brTarima; // override para que se guarde en la tarima BR
        // Inicializar micrófono igual que en Embarques
        if(typeof setupMic==='function') setupMic('btn-hall-mic','hall-mic-lang','hall-mic-status','hall-input');
        document.getElementById('modal-hallazgo').classList.add('open');
      });
    }

    // Botón Guardar
    document.getElementById('br-btn-save').addEventListener('click', function(){
      brSaveAll();
      var btn = this;
      btn.textContent = '✓ Guardado';
      btn.style.background = '#1e7e34';
      setTimeout(function(){ btn.textContent = 'Guardar conteo ✓'; btn.style.background=''; }, 1500);
    });

    // Exportar
    document.getElementById('br-btn-export').addEventListener('click', brExport);

    // Filtros reporte
    document.getElementById('br-filtro-clear').addEventListener('click', function(){
      document.getElementById('br-filtro-desde').value = '';
      document.getElementById('br-filtro-hasta').value = '';
      brRenderReporte();
    });
    document.querySelectorAll('.br-clasif-btn').forEach(function(btn){
      btn.addEventListener('click', function(){
        brClasifFiltro = this.dataset.clasif;
        document.querySelectorAll('.br-clasif-btn').forEach(function(b){
          b.style.background = b.dataset.clasif===brClasifFiltro ? '#1a1a1a' : 'transparent';
          b.style.color      = b.dataset.clasif===brClasifFiltro ? '#fff' : '#666';
        });
        brRenderReporte();
      });
    });
    ['br-filtro-desde','br-filtro-hasta'].forEach(function(id){
      var el = document.getElementById(id);
      if(el) el.addEventListener('change', brRenderReporte);
    });
  }
}

/* ── Selector de Tarima ──────────────────────────────────────────── */
function brRenderSelector(){
  var sel = document.getElementById('br-sel-tarima'); if(!sel) return;
  var brs = brConts();
  if(!brs.length){
    sel.innerHTML = '<option value="">Sin BoomRooms cargados — subí el teórico</option>';
    brTarima = null;
    document.getElementById('br-add-form').style.display = 'none';
    return;
  }
  sel.innerHTML = '<option value="">— Seleccioná una Tarima —</option>'
    + brs.map(function(c){ return '<option value="'+brEsc(c)+'"'+(c===brTarima?' selected':'')+'>'+brEsc(c)+'</option>'; }).join('');

  if(brTarima && brs.indexOf(brTarima) < 0) brTarima = null;
  sel.value = brTarima || '';

  // Mantener sincronizado el input de búsqueda con la tarima seleccionada
  // (no tocar mientras el usuario está escribiendo en ese mismo campo).
  var buscarInp = document.getElementById('br-tarima-buscar');
  if(buscarInp && document.activeElement !== buscarInp) buscarInp.value = brTarima || '';

  // Cargar fecha de trabajo de la tarima actual
  var fechaInp2 = document.getElementById('br-fecha-trabajo');
  if(fechaInp2){
    fechaInp2.value = (brTarima && teorico[brTarima] && teorico[brTarima].fechaCarga)
      ? teorico[brTarima].fechaCarga : '';
  }

  // Mostrar formulario si hay tarima
  document.getElementById('br-add-form').style.display = brTarima ? '' : 'none';

  // Mostrar tab asignaciones para supervisores
  var btnA = document.getElementById('br-tab-btn-asign');
  if(btnA) btnA.style.display = isSup() ? '' : 'none';
}

/* -- Render Conteo -- igual que Embarques: solo HTML, locks via brApplyConteoLocks -- */
function brRenderConteo(){
  brRenderSelector();
  var tbody = document.getElementById('br-tbody-conteo'); if(!tbody) return;
  var prog  = document.getElementById('br-prog-text');

  if(!brTarima || !teorico[brTarima]){
    tbody.innerHTML = '<tr><td colspan="7" class="empty">Selecciona una Tarima para comenzar.</td></tr>';
    if(prog) prog.textContent = '';
    return;
  }

  var items  = brItems(brTarima);
  var fis    = brFisArr(brTarima);
  var now    = Date.now();
  var myName = currentUser ? currentUser.name : '';
  var html   = '';
  var contadas = 0;
  ...
  items.forEach(function(it, i){
    var sv   = fis[i] || null;
    var fval = sv && sv.fisico !== null && sv.fisico !== undefined ? sv.fisico : '';
    var diff = fval !== '' ? Number(fval) - Number(it.qty) : null;
    var dc   = diff===null?'gray':diff===0?'green':diff>0?'blue':'red';
    var dv   = diff===null?'&mdash;':(diff>0?'+':'')+diff;
    var estado = diff===null?'&mdash;':diff===0
      ?'<span style="color:#1e7e34;font-weight:600">Exacto</span>'
      :diff>0?'<span style="color:#1565c0">Sobrante</span>'
      :'<span style="color:#c0392b">Faltante</span>';
    if(fval!=='') contadas++;

    // Lock display -- serverLocks applied dynamically by brApplyConteoLocks
    var srvLock = window.serverLocks ? window.serverLocks[brTarima+':'+i] : null;
    var locLock = locks[brTarima+'_'+i];
    var isLockedSrv = srvLock && srvLock.user !== myName;
    var isLockedLoc = locLock && locLock.quien !== myName && (now - locLock.at) < 90000;
    var isLocked = isLockedSrv || isLockedLoc;
    var lockWho  = isLockedSrv ? srvLock.user : (isLockedLoc ? locLock.quien : '');
    var lockLabel = isLocked ? '<div class="lock-label">' + lockWho + ' lo esta editando</div>' : '';
    var dis = isLocked ? 'disabled' : '';

    var quien  = sv && sv.quien ? sv.quien : '';
    var ts     = sv && sv.ts ? sv.ts : '';
    var ago    = sv && sv.lastAt ? Math.round((now - sv.lastAt)/1000) : null;
    var agoStr = ago !== null ? (ago < 60 ? ago+'s' : Math.round(ago/60)+'min') : '';
    var hint   = quien
      ? '<div class="trace-hint">' + brEsc(quien) + (ts?' - '+brEsc(ts):'') + '</div>'
        +(agoStr?'<div class="trace-hint">'+brEsc(quien)+' - '+agoStr+'</div>':'')
      : '';
    ...
    html += '<tr>'
      ...
      +'<td>'+brEsc(it.desc||'--')+'</td>'
      +'<td class="tc">'+it.qty+'</td>'
      +'<td class="tc"><input type="number" min="0" step="1" value="'+fval+'" id="br-fi'+i+'" data-i="'+i+'" '+dis
        +' style="width:80px;padding:5px 8px;border:1px solid #ccc;border-radius:6px;text-align:center;font-size:.88rem">'
        +hint+lockLabel+'</td>'
      +'<td class="tc '+dc+'" id="br-d'+i+'">'+dv+'</td>'
      +'<td class="tc" id="br-est'+i+'">'+estado+'</td>'
      +'<td class="tc">--</td>'
      +'</tr>';
  });

  for(var j=items.length; j<fis.length; j++){
    var ex = fis[j]; if(!ex || !ex.extraSku) continue;
    var exQty = ex.fisico !== null && ex.fisico !== undefined ? ex.fisico : '--';
    html += '<tr style="background:#fffef0">'
      +'<td>'+brEsc(ex.sku||'--')+'</td>'
      +'<td>'+brEsc(ex.desc||'--')+'</td>'
      +'<td class="tc" style="color:#aaa">--</td>'
      +'<td class="tc" style="font-weight:600">'+exQty+'</td>'
      +'<td class="tc blue">+'+(exQty!=='--'?exQty:0)+'</td>'
      +'<td class="tc"><span style="color:#1565c0">Sobrante</span></td>'
      +'<td class="tc"><button class="btn btn-sm br-del-extra" data-br-del="'+j
        +'" style="background:#fdecea;color:#c0392b;border:none;border-radius:6px;padding:4px 10px;cursor:pointer;font-size:.75rem">DEL</button></td>'
      +'</tr>';
  }

  tbody.innerHTML = html || '<tr><td colspan="7" class="empty">Sin SKUs en esta tarima.</td></tr>';
  if(prog) prog.textContent = contadas+' / '+items.length+' teoricos contados';

  // Botones eliminar extra
  tbody.querySelectorAll('.br-del-extra').forEach(function(btn){
    if(btn._brSrv) return;
    btn._brSrv = 1;
    btn.addEventListener('click', function(){
      var idx = parseInt(this.dataset.brDel);
      if(!confirm('Eliminar esta linea extra?')) return;
      fisico[brTarima].splice(idx, 1);
      brSaveAll();
      brRenderConteo();
    });
  });

  // Aplicar locks y registrar listeners -- igual que el patch de Embarques
  brApplyConteoLocks();

  // FIX (jul-2026): reaplicar el filtro de búsqueda SKU tras repintar la tabla
  // (mismo motivo que en Embarques: un poll puede disparar brRenderConteo y
  // "perder" visualmente el filtro si no se reaplica).
  if (typeof window.applyBrSkuSearchFilter === 'function') window.applyBrSkuSearchFilter();
}

/* -- brApplyConteoLocks -- identico al patch renderConteo de Embarques --
   Corre despues de cada brRenderConteo Y en cada poll via _brRefreshIfActive.
   1. Aplica locks en vivo sobre inputs existentes (sin recrear HTML)
   2. Registra listeners UNA SOLA VEZ con _brSrv flag -- igual que fi._srv de Embarques
   Usa myLocks global (mismo objeto de Embarques), /api/lock y /api/unlock.
-- */
var brSaveTimers = {};
function brApplyConteoLocks(){
  if(!brTarima || !teorico[brTarima]) return;
  var items  = brItems(brTarima);
  var myName = currentUser ? currentUser.name : '';

  for(var ii=0; ii<items.length; ii++){
    (function(idx){
      var fi = document.getElementById('br-fi'+idx);
      if(!fi) return;

      // 1. Lock state en vivo -- igual que Embarques patch post-renderConteo
      var lock = window.serverLocks ? window.serverLocks[brTarima+':'+idx] : null;
      var isLockedByOther = lock && lock.user !== myName;
      if(isLockedByOther){
        fi.disabled = true;
        fi.style.borderColor = '#e0c96e';
        fi.style.background  = '#fffef0';
        var par = fi.parentNode;
        if(par && !par.querySelector('.br-srv-lock')){
          var badge = document.createElement('div');
          badge.className = 'br-srv-lock';
          badge.style.cssText = 'font-size:.68rem;color:#e67e22;margin-top:2px';
          badge.textContent = lock.user + ' contando...';
          par.appendChild(badge);
        }
      } else {
        if(!myLocks[brTarima+':'+idx]) {
          fi.disabled = false;
          fi.style.borderColor = '';
          fi.style.background  = '';
        }
        var par2 = fi.parentNode;
        if(par2){ var bl=par2.querySelector('.br-srv-lock'); if(bl) par2.removeChild(bl); }
      }

      // 2. Listeners UNA SOLA VEZ con _brSrv -- igual que Embarques fi._srv
      if(fi._brSrv) return;
      fi._brSrv = 1;

      fi.addEventListener('focus', function(){
        var key = brTarima+':'+idx;
        if(myLocks[key]) return;
        fetch('/api/lock',{method:'POST',headers:{'Content-Type':'application/json'},
          body:JSON.stringify({cont:brTarima, idx:idx, user:currentUser.name})})
          .then(function(r){return r.json();})
          .then(function(d){
            if(d.ok){
              myLocks[key] = true;
              setTimeout(function(){
                ...
                delete myLocks[key];
                fetch('/api/unlock',{method:'POST',headers:{'Content-Type':'application/json'},
                  body:JSON.stringify({cont:brTarima, idx:idx, user:currentUser.name})}).catch(function(){});
              }, 90000);
            }
          }).catch(function(){});
      });
        ...
        var val = this.value==='' ? null : parseFloat(this.value);
        if(!Array.isArray(fisico[brTarima])) fisico[brTarima] = new Array(items.length).fill(null);
        var it3 = items[idx];
        fisico[brTarima][idx] = {
          fisico:   val,
          daniado:  null,
          cobertura: val!==null?'Auditado':'En revision',
          quien:    currentUser?currentUser.name:'',
          lastUser: currentUser?currentUser.name:'',
          ts:       new Date().toLocaleTimeString('es-GT',{hour:'2-digit',minute:'2-digit'}),
          lastAt:   Date.now()
        };
        if(it3){
          var d3 = val!==null ? val-Number(it3.qty) : null;
          var dEl = document.getElementById('br-d'+idx);
          if(dEl) dEl.textContent = d3===null?'--':(d3>0?'+':'')+d3;
          // FIX (jul-2026): actualizar Estado en el mismo instante que Diferencia.
          // Antes Estado solo se recalculaba en el próximo brRenderConteo()
          // completo (al guardar/poll/blur), por eso se percibía "lento" o
          // desincronizado respecto a la Diferencia, que sí era inmediata.
          var estEl = document.getElementById('br-est'+idx);
          if(estEl){
            estEl.innerHTML = d3===null ? '&mdash;'
              : d3===0 ? '<span style="color:#1e7e34;font-weight:600">Exacto</span>'
              : d3>0   ? '<span style="color:#1565c0">Sobrante</span>'
              :          '<span style="color:#c0392b">Faltante</span>';
          }
        }
        clearTimeout(brSaveTimers[idx]);
        brSaveTimers[idx] = setTimeout(function(){ brSaveField(idx); }, 2500);
      });
    })(ii);
  }
}
...
function brSaveField(idx){
  if(!brTarima || !currentUser) return;
  var sv = (fisico[brTarima]||[])[idx];
  if(!sv) return;
  fetch('/api/conteo/field',{
    method:'POST', headers:{'Content-Type':'application/json'},
    body: JSON.stringify({
      ...
      fisico:    sv.fisico,
      daniado:   null,
      cobertura: sv.cobertura||'Auditado',
      usuario:   currentUser.name
    })
  }).then(function(r){ return r.json(); })
    .then(function(d){ if(d.ok) lastVersion = d.version; })
...
/* ── Guardar todo el array (extra SKUs) ──────────────────────────── */
function brSaveAll(){
  if(!brTarima || !currentUser) return;
  fetch('/api/conteo/field',{
    method:'POST', headers:{'Content-Type':'application/json'},
...
/* ── Render Reporte ──────────────────────────────────────────────── */
function brRenderReporte(){
  var tbody = document.getElementById('br-tbody-reporte'); if(!tbody) return;
  var brs   = brConts();
  var fdStr = (document.getElementById('br-filtro-desde')||{value:''}).value;
  var fhStr = (document.getElementById('br-filtro-hasta')||{value:''}).value;

  var filtrados = brs.filter(function(t){
    var cl = brClasif(t);
    if(brClasifFiltro!=='todos' && cl!==brClasifFiltro) return false;
    var fc = teorico[t] && teorico[t].fechaCarga;
    if(fdStr && (!fc || fc < fdStr)) return false;
    if(fhStr && (!fc || fc > fhStr)) return false;
    return true;
  });

  if(!filtrados.length){
    tbody.innerHTML = '<tr><td colspan="12" class="empty">Sin BoomRooms para el filtro seleccionado.</td></tr>';
    return;
  }
  ...
  filtrados.forEach(function(t){
    var items = brItems(t);
    var fis   = brFisArr(t);
    var exactos=0, sobrantes=0, faltantes=0, costoTotal=0, itemsFinales=items.length;

    items.forEach(function(it,i){
      var sv  = fis[i];
      var fv  = sv && sv.fisico!==null && sv.fisico!==undefined ? Number(sv.fisico) : null;
      var cu  = Number(it.costo||0);
      costoTotal += cu * Number(it.qty||0);
      if(fv===null) return;
      var d = fv - Number(it.qty||0);
      if(d===0) exactos++; else if(d>0) sobrantes++; else faltantes++;
    });
    for(var j=items.length;j<fis.length;j++){
      var ex=fis[j]; if(!ex||!ex.extraSku) continue;
      itemsFinales++; sobrantes++;
    }

    var contadas = exactos+sobrantes+faltantes;
    var cob  = items.length ? Math.round((contadas/items.length)*100) : 0;
    var ira  = items.length ? (exactos/items.length*100).toFixed(1)+'%' : '—';
    var cl   = brClasif(t);
    var badge = cl==='Auditado'
      ? '<span style="background:#eafaf1;color:#1e7e34;font-size:.72rem;padding:2px 8px;border-radius:8px">Auditado</span>'
      : '<span style="background:#fef9e7;color:#b7770d;font-size:.72rem;padding:2px 8px;border-radius:8px">Pendiente</span>';
    // FIX (ago-2026): Responsable = nombres asignados a esta tarima en la
    // pestaña Asignaciones (asignaciones[t]), mismo dato que ya usan
    // Embarques/Traslados — solo que ahí no se mostraba en Reporte.
    var responsable = (asignaciones[t] || []).join(', ') || '—';

    html += '<tr>'
      +'<td style="font-weight:600">'+brEsc(t)+'</td>'
      +'<td class="tc">'+items.length+'</td>'
      +'<td class="tc">'+itemsFinales+'</td>'
      +'<td class="tc green">'+exactos+'</td>'
      +'<td class="tc blue">'+sobrantes+'</td>'
      +'<td class="tc red">'+faltantes+'</td>'
      +'<td class="tc">'+cob+'%</td>'
      +'<td class="tc">'+ira+'</td>'
      +'<td class="tc" style="font-family:monospace">Q'+Math.round(costoTotal).toLocaleString('es-GT')+'</td>'
      +'<td style="font-size:.78rem">'+brFechaTrabajo(t)+'</td>'
      +'<td style="font-size:.78rem">'+brEsc(responsable)+'</td>'
      +'<td>'+badge+'</td>'
      +'</tr>';
  });
...
/* ── Hallazgos BoomRoom ──────────────────────────────────────────── */
// FIX (jul-2026, v2): reescrita de forma totalmente independiente.
// La versión anterior reasignaba temporalmente el array global `hallazgos`
// y movía el <div id="hall-list"> de Embarques dentro de #br-hall-list
// para reusar renderHallazgosList(). Eso dependía de un flag de contexto
// (_brHallazgosContext) para que ese mismo filtro no se auto-anulara, y
// terminó siendo frágil: alcanzaba con que brConts() devolviera algo
// inesperado en ese instante para que el resultado quedara vacío en los
// dos lados. Ahora esta función NUNCA toca `hallazgos` ni mueve nodos —
// solo lee y pinta directo en #br-hall-list, con su propio HTML (mismo
// diseño de tarjeta que Embarques).
function brRenderHallazgos(){
  var brList = document.getElementById('br-hall-list'); if(!brList) return;

  var brs = brConts();
  var visibleHall = (hallazgos||[]).filter(function(h){
    return h.tipo !== 'CDG' && h.cont && brs.indexOf(h.cont) >= 0;
  });

  if(!visibleHall.length){
    brList.innerHTML = '<p class="empty" style="color:#aaa;text-align:center;padding:1rem;font-size:.85rem">Sin hallazgos registrados aún.</p>';
    return;
  }

  var toolbar = '<div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px;margin-bottom:.75rem">'
    +'<span style="font-size:.85rem;color:#666">Total: <strong>'+visibleHall.length+'</strong> hallazgo'+(visibleHall.length!==1?'s':'')+' en '+Object.keys(visibleHall.reduce(function(a,h){a[h.cont]=1;return a;},{})).length+' tarima(s)</span>'
    +(typeof exportHallazgosWord==='function' ? '<button class="btn btn-sm btn-blue" id="br-btn-hall-word">📥 Exportar Word</button>' : '')
    +'</div>';

  var byCont = {};
  visibleHall.forEach(function(h){ (byCont[h.cont] = byCont[h.cont]||[]).push(h); });
  var groupKeys = Object.keys(byCont).sort();
  var globalIdx = 0;
  var html = toolbar;
  groupKeys.forEach(function(cont){
    html += '<div class="sec" style="margin-top:1rem;margin-bottom:.5rem">📦 '+brEsc(cont)+' <span style="font-weight:400;color:#888;font-size:.8rem">('+byCont[cont].length+')</span></div>';
    byCont[cont].forEach(function(hh){
      globalIdx++;
      var metaLine = [];
      if(hh.fecha)        metaLine.push('📅 '+brEsc(hh.fecha));
      if(hh.turno)        metaLine.push('⏱ '+brEsc(hh.turno));
      if(hh.area)         metaLine.push('📍 '+brEsc(hh.area));
      if(hh.tipoHallazgo) metaLine.push('🏷 '+brEsc(hh.tipoHallazgo));
      html += '<div class="card" style="margin-bottom:.75rem">'
        +'<div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:6px;margin-bottom:.5rem">'
        +'<span class="badge b-diff">⚠ Hallazgo #'+globalIdx+'</span>'
        +'<span style="font-size:.75rem;color:#aaa">'+brEsc(hh.autor||'—')+' · '+brEsc(hh.ts||'')+'</span>'
        +'</div>'
        +(metaLine.length?'<p style="font-size:.72rem;color:#666;margin-bottom:.4rem">'+metaLine.join(' · ')+'</p>':'')
        +'<p style="font-size:.875rem;font-weight:700;margin-bottom:.4rem">'+brEsc(hh.quePaso||hh.desc||'')+'</p>'
        +(hh.condicion?'<p style="font-size:.82rem;margin-bottom:.2rem"><strong>Condición:</strong> '+brEsc(hh.condicion)+'</p>':'')
        +(hh.criterio?'<p style="font-size:.82rem;margin-bottom:.2rem"><strong>Criterio:</strong> '+brEsc(hh.criterio)+'</p>':'')
        +(hh.causa?'<p style="font-size:.82rem;margin-bottom:.2rem"><strong>Causa:</strong> '+brEsc(hh.causa)+'</p>':'')
        +(hh.efecto?'<p style="font-size:.82rem;margin-bottom:.2rem"><strong>Efecto:</strong> '+brEsc(hh.efecto)+'</p>':'')
        +(hh.recomendacion?'<p style="font-size:.82rem"><strong>Recomendación:</strong> '+brEsc(hh.recomendacion)+'</p>':'')
        +(hh.foto?'<img src="'+hh.foto+'" style="max-width:100%;max-height:120px;border-radius:8px;margin-top:6px;border:1px solid #e0e0e0">':'')
        +(hh.modificadoPor?'<p style="font-size:.72rem;color:#aaa;margin-top:4px">Modificado por '+brEsc(hh.modificadoPor)+' · '+brEsc(hh.modificadoTs)+'</p>':'')
        +'<div style="margin-top:8px;display:flex;gap:6px;flex-wrap:wrap"><button class="btn btn-sm btn-warn br-edit-hall-btn" data-id="'+brEsc(hh.id)+'" style="font-size:.78rem;min-height:34px;padding:4px 12px">✏️ Modificar</button>'
        +((typeof isHallazgoAdmin==='function' && isHallazgoAdmin())?'<button class="btn btn-sm br-del-hall-btn" data-id="'+brEsc(hh.id)+'" style="font-size:.78rem;min-height:34px;padding:4px 12px;background:#c0392b;color:#fff">🗑 Eliminar</button>':'')
        +'</div>'
        +'</div>';
    });
  });
  ...
  var btnWord = document.getElementById('br-btn-hall-word');
  if(btnWord && typeof exportHallazgosWord==='function') btnWord.addEventListener('click', exportHallazgosWord);

  brList.querySelectorAll('.br-edit-hall-btn').forEach(function(btn){
    btn.addEventListener('click', function(){
      if(typeof openHallazgoModal==='function') openHallazgoModal(this.dataset.id);
    });
  });
  brList.querySelectorAll('.br-del-hall-btn').forEach(function(btn){
    btn.addEventListener('click', function(){
      if(typeof isHallazgoAdmin==='function' && !isHallazgoAdmin()){ alert('Sin permiso para eliminar.'); return; }
      var id = this.dataset.id;
      var h = hallazgos.find(function(x){ return x.id===id; });
      var label = h ? (h.cont+' — '+(h.quePaso||h.desc||'(sin descripción)').substring(0,60)) : id;
      if(!confirm('¿Eliminar este hallazgo?\n\n'+label+'\n\nEsta acción NO se puede deshacer.')) return;
      hallazgos = hallazgos.filter(function(x){ return x.id !== id; });
      if(typeof deleteHallazgoDirectToSupabase==='function') deleteHallazgoDirectToSupabase(id);
      fetch('/api/hallazgo',{method:'POST',headers:{'Content-Type':'application/json'},
        body:JSON.stringify({action:'delete', id:id})})
        .then(function(r){return r.json();})
        .then(function(d){if(d&&d.ok)lastVersion=d.version;})
        .catch(function(){});
      if(typeof addHist==='function') addHist('Eliminó hallazgo '+(h?h.cont:id));
      brRenderHallazgos();
      if(typeof flashMsg==='function') flashMsg('status-bar','ok','✅ Hallazgo eliminado.');
    });
  });
}


function brRenderAsign(){
  var div = document.getElementById('br-asign-content'); if(!div) return;
  var brs = brConts();
  var allPeople = (typeof SUPERVISORES!=='undefined'?SUPERVISORES:[])
    .concat(typeof CONTADORES_LIST!=='undefined'?CONTADORES_LIST:[]);
  var html = '';

  // Asignación de tarimas BoomRoom
  html += '<div class="collapsible-hdr" id="br-asign-hdr-main"><span class="ch-title">Asignación de tarimas BoomRoom</span><span class="ch-ico">▼</span></div>';
  html += '<div id="br-asign-body-main" class="collapsible-body">';
  if(!brs.length){ html += '<p class="empty">Cargá el teórico BoomRoom primero.</p>'; }
  else{
    brs.forEach(function(t){
      var assigned = asignaciones[t]||[];
      var fis = fisico[t];
      var estado = fis ? '<span class="badge b-ok">Contado</span>'
        : assigned.length ? '<span class="badge b-diff">En proceso</span>'
        : '<span class="badge b-pend">Pendiente</span>';
      html += '<div class="asign-row">'
        +'<div style="flex:1;min-width:120px"><strong style="font-size:.875rem">'+brEsc(t)+'</strong>'
        +'<div style="font-size:.75rem;color:#aaa">BoomRoom</div></div>'
        +'<div style="display:flex;flex-wrap:wrap;gap:4px;flex:2">'
        +assigned.map(function(a){return'<span class="asign-chip">'+brEsc(a)+'<button data-br-cont="'+brEsc(t)+'" data-name="'+brEsc(a)+'" class="br-btn-rm-asign">×</button></span>';}).join('')
        +'</div>'
        +'<div style="display:flex;gap:6px;align-items:center;flex-shrink:0">'+estado
        +'<select class="br-asign-sel" data-cont="'+brEsc(t)+'" style="font-size:.8rem;padding:5px 8px;border:1px solid #ccc;border-radius:8px;min-height:36px;max-width:150px">'
        +'<option value="">+ Asignar</option>'
        +allPeople.filter(function(p){return assigned.indexOf(p)<0;}).map(function(p){return'<option value="'+brEsc(p)+'">'+brEsc(p)+'</option>';}).join('')
        +'</select></div></div>';
    });
  }
  html += '</div>';
  ...
  // Historial del día
  html += '<div class="collapsible-hdr" id="br-asign-hdr-hist" style="margin-top:1rem"><span class="ch-title">Historial del día</span><span class="ch-ico">▼</span></div>';
  html += '<div id="br-asign-body-hist" class="collapsible-body">';
  if(!historial||!historial.length){ html += '<p class="empty">Sin actividad.</p>'; }
  else{
    html += '<div style="max-height:300px;overflow-y:auto">';
    historial.slice().reverse().forEach(function(h){
      html += '<div class="hist-item"><span class="hist-time">'+brEsc(h.ts||'')+'</span> <strong>'+brEsc(h.usuario||'')+'</strong> — '+brEsc(h.accion||'')+'</div>';
    });
    html += '</div>';
  }
  html += '</div>';
  ...
  // Collapsibles
  ['main','hist'].forEach(function(k){
    var hdr  = div.querySelector('#br-asign-hdr-'+k);
    var body = div.querySelector('#br-asign-body-'+k);
    if(!hdr||!body) return;
    hdr.addEventListener('click', function(){
      var open = body.style.display!=='none';
      body.style.display = open?'none':'';
      var ico = hdr.querySelector('.ch-ico');
      if(ico) ico.textContent = open?'▶':'▼';
    });
  });

  // Wire asign selects
  div.querySelectorAll('.br-asign-sel').forEach(function(sel){
    sel.addEventListener('change', function(){
      var t = this.dataset.cont; var name = this.value; if(!name) return;
      if(!asignaciones[t]) asignaciones[t]=[];
      if(asignaciones[t].indexOf(name)<0) asignaciones[t].push(name);
      fetch('/api/asignar',{method:'POST',headers:{'Content-Type':'application/json'},
        body:JSON.stringify({cont:t, nombre:name, usuario:currentUser.name})}).catch(function(){});
      brRenderAsign();
    });
  });

  // Wire remove buttons
  div.querySelectorAll('.br-btn-rm-asign').forEach(function(btn){
    btn.addEventListener('click', function(){
      var t = this.dataset.brCont; var name = this.dataset.name;
      if(!asignaciones[t]) return;
      asignaciones[t] = asignaciones[t].filter(function(a){ return a!==name; });
      fetch('/api/asignar',{method:'POST',headers:{'Content-Type':'application/json'},
        body:JSON.stringify({cont:t, nombre:name, action:'remove', usuario:currentUser.name})}).catch(function(){});
      brRenderAsign();
    });
  });
}
...
function brExport(){
  var brs = brConts();
  if(!brs.length){ alert('No hay BoomRooms cargados.'); return; }
  var today = new Date();
  var ds = today.getFullYear()+'_'+String(today.getMonth()+1).padStart(2,'0')+'_'+String(today.getDate()).padStart(2,'0');
  var wb = XLSX.utils.book_new();

  // Hoja Detalle
  var det = [['Fecha auditoría','Tipo SKU','SKU','Descripción','Ubicación Boomroom',
    'Teórico','Físico','Diferencia','Diferencia ABS','Costo unitario',
    'Costo diferencia Q','Costo diferencia Q (ABS)','Estatus','Responsable']];

  brs.forEach(function(t){
    var items = brItems(t);
    var fis   = brFisArr(t);
    var fecha = brFechaTrabajo(t) || today.toLocaleDateString('es-GT');
    // FIX (ago-2026): Responsable = nombres asignados en Asignaciones
    var responsableDet = (asignaciones[t] || []).join(', ') || '—';
    items.forEach(function(it,i){
      var sv  = fis[i];
      var fv  = sv && sv.fisico!==null && sv.fisico!==undefined ? Number(sv.fisico) : null;
      var cu  = Number(it.costo||0);
      var d   = fv!==null ? fv-Number(it.qty||0) : null;
      var est = fv!==null ? (d===0?'Exacto':d>0?'Sobrante':'Faltante') : 'Pendiente';
      det.push([fecha,'Teórico',it.sku,it.desc||'',t,
        Number(it.qty||0), fv!==null?fv:'', d!==null?d:'', d!==null?Math.abs(d):'',
        cu||'', d!==null&&cu?cu*d:'', d!==null&&cu?Math.abs(cu*d):'', est, responsableDet]);
    });
    for(var j=items.length;j<fis.length;j++){
      var ex=fis[j]; if(!ex||!ex.extraSku) continue;
      var fvEx=ex.fisico!==null&&ex.fisico!==undefined?Number(ex.fisico):0;
      det.push([fecha,'Agregado',ex.sku||'',ex.desc||'',t,
        0,fvEx,fvEx,fvEx,'','','','Sobrante',responsableDet]);
    }
  });
  XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet(det), 'Detalle BoomRoom');

  // Hoja Resumen
  var sum = [['Tarima (BoomRoom)','Ítems teórico','Ítems finales','Exactos',
    'Sobrantes','Faltantes','% Cobertura','IRA','Costo total Q','Fecha trabajo','Responsable','Clasificación']];
  brs.forEach(function(t){
    var items=brItems(t); var fis=brFisArr(t);
    var ex=0,so=0,fa=0,ct=0,fin=items.length;
    items.forEach(function(it,i){
      var sv=fis[i]; var fv=sv&&sv.fisico!==null&&sv.fisico!==undefined?Number(sv.fisico):null;
      ct+=Number(it.costo||0)*Number(it.qty||0);
      if(fv===null) return;
      var d=fv-Number(it.qty||0);
      if(d===0)ex++; else if(d>0)so++; else fa++;
    });
    for(var j=items.length;j<fis.length;j++){var e=fis[j];if(!e||!e.extraSku)continue;fin++;so++;}
    var cnt=ex+so+fa;
    // FIX (ago-2026): Responsable = nombres asignados en la pestaña
    // Asignaciones (asignaciones[t]), igual que ahora se muestra en pantalla.
    var responsable = (asignaciones[t] || []).join(', ') || '—';
    sum.push([t,items.length,fin,ex,so,fa,
      items.length?Math.round(cnt/items.length*100)+'%':'0%',
      items.length?(ex/items.length*100).toFixed(1)+'%':'—',
      Math.round(ct),brFechaTrabajo(t),responsable,brClasif(t)]);
  });
  XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet(sum), 'Resumen BoomRoom');
  XLSX.writeFile(wb, 'Reporte_BoomRoom_'+ds+'.xlsx');
}

/* ── Exponer funciones globalmente ───────────────────────────────── */
window.brInit             = brInit;
window.brConts            = brConts;
window.brRenderHallazgos  = brRenderHallazgos;
window._brRefreshIfActive = function(){
  var pg = document.getElementById('pg-boomroom');
  if(!pg || !pg.classList.contains('active')) return;
  var conteoDiv = document.getElementById('br-tab-conteo');
  if(conteoDiv && conteoDiv.style.display !== 'none'){
    brRenderSelector();
    brRenderConteo();
    // Aplicar locks en vivo sin esperar al proximo render completo
    if(typeof brApplyConteoLocks === 'function') brApplyConteoLocks();
  }
  var reporteDiv = document.getElementById('br-tab-reporte');
  if(reporteDiv && reporteDiv.style.display !== 'none') brRenderReporte();
  // FIX (jul-2026): también refrescar Hallazgos de BoomRoom si esa pestaña
  // está activa — antes solo se refrescaba al hacer click en la pestaña,
  // así que un hallazgo nuevo (propio o de otro usuario) no aparecía hasta
  // que el usuario cambiaba de pestaña y volvía.
  var hallDiv = document.getElementById('br-tab-hallazgos');
  if(hallDiv && hallDiv.style.display !== 'none') brRenderHallazgos();
};
